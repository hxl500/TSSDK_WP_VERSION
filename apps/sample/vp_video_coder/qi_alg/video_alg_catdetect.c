...
typedef struct {
    int initialized;
    catdetect_config_t config;
} catdetect_handle_t;

void* video_alg_catdetect_create(void)
{
    catdetect_handle_t* handle = (catdetect_handle_t*)malloc(sizeof(catdetect_handle_t));
    if(handle == NULL)
    {
    ...
    memset(handle, 0, sizeof(catdetect_handle_t));

    // 设置默认配置
    handle->config.enable = 1;
    handle->config.threshold = 80;
    handle->config.min_width = 50;
    handle->config.min_height = 50;

    handle->initialized = 1;

    printf("cat detect algorithm created\n");
...
void video_alg_catdetect_destroy(void* handle)
{
    ...
    catdetect_handle_t* cat_handle = (catdetect_handle_t*)handle;

    if(cat_handle->initialized)
    {
        printf("cat detect algorithm destroyed\n");
        free(cat_handle);
    }
}