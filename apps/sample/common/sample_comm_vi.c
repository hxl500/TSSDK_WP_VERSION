...
static TS_S32 SAMPLE_COMM_VI_DestroySingleVi(SAMPLE_VI_INFO_S* pstViInfo)
{
    ...
    s32Ret = SAMPLE_COMM_VI_StopViChn(pstViInfo);
    if (s32Ret != TS_SUCCESS)
    {
        SAMPLE_PRT("SAMPLE_COMM_VI_StopViChn failed !\n");
		return TS_FAILURE;
    }

    s32Ret = SAMPLE_COMM_VI_StopViPipe(pstViInfo);
    if (s32Ret != TS_SUCCESS)
    {
        SAMPLE_PRT("SAMPLE_COMM_VI_StopViPipe failed !\n");
		return TS_FAILURE;
    }
...
TS_S32 SAMPLE_COMM_VI_CreateIsp(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    ...
    for (i = 0; i < pstViConfig->s32WorkingViNum; i++)
    {
		...
		SAMPLE_PRT("SAMPLE_COMM_VI_StartIsp of vipipe[%d] success !\n", pstViInfo->stPipeInfo.aPipe);
    }
...
TS_S32 SAMPLE_COMM_VI_StopVi(SAMPLE_VI_CONFIG_S* pstViConfig)
{
	...
	SAMPLE_PRT("SAMPLE_COMM_VI_DestroyIsp success!\n");
	...
	SAMPLE_PRT("SAMPLE_COMM_VI_DestroyVi success!\n");
