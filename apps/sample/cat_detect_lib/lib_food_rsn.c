...
static TS_VOID *pFoodRsnHandle = NULL;
extern TS_BOOL gRneOff;
...
extern TS_S32 TS_ALG_RSN_Init(TS_VOID **handle, ALG_MODEL_INIT_S *param);
...
TS_S32 ALG_RSN_Detect_FOOD_Init()
{
	...
	TS_U8 *weightpath = model_info->food_model_weight;
	TS_U8 *cfgpath = model_info->food_model_cfg;
	

    printf("(%s)%d#########111111111111111111111111111111111##################\n", __func__, __LINE__);
    printf("weightpath=%s\n",weightpath);
    printf("cfgpath=%s\n",cfgpath);
