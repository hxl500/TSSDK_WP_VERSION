...
int __weak eth_switch_test_entry(int argc, char *const argv[])
{
...
static int do_switch(struct cmd_tbl *cmdtp, int flag, int argc,
		  char *const argv[])
{
	return eth_switch_test_entry(argc, argv);
}

U_BOOT_CMD(
	switch, 5, 1, do_switch,
	"switch config and rw",
	"switch vlanmng <port> <tagged>      - add port to manage vlan with panel port\n"
	"switch vlanint                      - config vlan inter\n"
	"switch vlanwan                      - config vlan wan(wan port and mpu port)\n"
	"switch vlanwan                      - config vlan lan(lan port and mpu port)\n"
	"switch vlanget <vid>                - get vlan config and port info\n"
	"switch porten <port>                - enable port\n"
	"switch portdis <port>               - disable port\n"
	"switch portloop <port>              - enable port loopback\n"
	"switch portunloop <port>            - disable port loopback\n"
	"switch link                         - show all port link status\n"
	"switch statall <port>               - show port statistics verbous\n"
	"switch statsim <port>               - show port statistics simply\n"
	"switch statrst <port>               - clear port statistics\n"
);
