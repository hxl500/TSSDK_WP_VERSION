...
static int ts_compare_regions(volatile unsigned long *bufa,
			   volatile unsigned long *bufb, size_t count)
{
	volatile unsigned long  *p1 = bufa;
	volatile unsigned long  *p2 = bufb;
	...
	for (i = 0; i < count; i++, p1++, p2++) {
		if (*p1 != *p2) {
			printf("FAILURE: 0x%08lx != 0x%08lx (delta=0x%08lx -> bit %ld) at offset 0x%08lx(%p %p)\n",
			       (unsigned long)*p1, (unsigned long)*p2,
			       *p1 ^ *p2, __ffs(*p1 ^ *p2),
				(unsigned long)(i * sizeof(unsigned long)), p1, p2);
#if 0
			...
			if (*p1 != *p2) {
				if (*p1 == *p2){
					...
					printf("0x%08lx 0x%08lx 2 read error\r\n", *p1, *p2);
				}
			} else {
				printf("correct!!!!!!!!!\r\n");
				printf("0x%08lx 0x%08lx \r\n", *p1, *p2);
...
//#define FIX_BIT   17 //sikp bit test...

static ulong ts_test_bitflip_comparison(volatile unsigned long *bufa,
				     volatile unsigned long *bufb, size_t count, ulong start)
{
	volatile unsigned long *p1 = bufa;
	volatile unsigned long *p2 = bufb;
	unsigned int j, k;
	unsigned long q;
	unsigned short __maybe_unused tmp = 0, tmp_addr, addr_l, addr_h;
	size_t i;
	int max;
	int errs = 0;
	...
	for (k = 0; k < max; k++) {
		...
		for (j = 0; j < 8; j++) {
			...
			///icache_enable();
			//invalidate_icache_all();
			//*p1++ = *p2++ = (i % 2) == 0 ? (q & (0xffffffffL << 32 | (start + i * 0x8))) : ~q;
#endif
			...
			addr_h = (tmp_addr >> 16);
			for (i = 0; i < count; i++){
				tmp = ((((uint64_t)addr_h << 16) | addr_h) << 32) | (addr_l << 16) | addr_l;
				if ((i % 4) == 0)
					*p1++ = *p2++ = tmp;
				if ((i % 4) == 1)
					*p1++ = *p2++ = ~q;
			...
			errs += ts_compare_regions(bufa, bufb, count);
			if (errs) {
				return errs;
			}
		}
...
static ulong ts_mem_test_bitflip(vu_long *buf, ulong start, ulong end)
{
	/*
	 * Split the specified range into two halves.
	 * Note that mtest range is inclusive of start,end.
	...
	return ts_test_bitflip_comparison(buf, buf + half_size, half_size, start);
}
 ...
 */
static int ts_do_mem_mtest(struct cmd_tbl *cmdtp, int flag, int argc,
			char *const argv[])
{
	ulong start, end;
	...
	start = 0x00200000;//CONFIG_SYS_MEMTEST_START;
	end = 0x00300000;//CONFIG_SYS_MEMTEST_END;
...
//#define _MEM_END 0x80000000
#define _MEM_END 0xE0000000

	//end = gd->ram_size;

#ifdef FIX_BIT
uint32_t test_size = 1<<(FIX_BIT + 1);
#else
uint32_t test_size = end - start;//0x100000; //1MB
#endif

	for (; end < _MEM_END; start += test_size, end += test_size) {
		count = errs = 0;
		...
		for (iteration = 0;
			...
			if (IS_ENABLED(CONFIG_SYS_ALT_MEMTEST)) {
				#ifdef FIX_BIT
				errs = mem_test_alt(buf, start, start + (1<<FIX_BIT), dummy);
				#else
				errs = mem_test_alt(buf, start, end, dummy);
				//errs = mem_test_alt(buf, start, start + end, dummy);
				#endif
				if (errs == -1UL)
					break;
				if (IS_ENABLED(CONFIG_SYS_ALT_MEMTEST_BITFLIP)) {
					count += errs;
					errs = ts_mem_test_bitflip(buf, start, end);
				}
			} else {
...
U_BOOT_CMD(
	mtest_loop,	5,	1,	ts_do_mem_mtest,
	"simple RAM read/write test",
	"[start [end [pattern [iterations]]]]"
);
#endif	/* CONFIG_CMD_MEMTEST */
