...
__weak int boot_select_is_ospi_nand(void)
{
	return 1;
}

__weak int boot_select_is_emmc(void)
{
	return 1;
}

__weak int boot_select_is_rawnand(void)
{
	return 1;
}
...
{
	if (boot_select_is_rawnand()) {
		puts("NAND:  ");
		nand_init();
		printf("%lu MiB\n", nand_size() / 1024);
	}

	//if (boot_select_is_ospi_nand()) {
		puts("SPI-NAND:  ");
		spinand_init_r();
		printf("%lu MiB\n", nand_size() / 1024);
	//}
