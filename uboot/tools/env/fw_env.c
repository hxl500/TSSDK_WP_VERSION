...
int fw_env_open(struct env_opts *opts)
{
	...
	if (!have_redund_env) {
		if (!crc0_ok) {
			fprintf(stderr,
				"Warning: 22 Pram Bad CRC, using default environment ,sizeof(default_environment):%u,ENV_SIZE:%lu\n",sizeof(default_environment),ENV_SIZE);
		...
		if (crc0_ok && !crc1_ok) {
			...
			fprintf(stderr,
				"Warning:1 Parm Bad CRC, using default environment,default_environment:%u\n",sizeof(default_environment));
			memcpy(environment.data, default_environment, sizeof(default_environment));
			environment.dirty = 1;
			dev_current = 0;
		} else {
