...
#if CONFIG_IS_ENABLED(CLK)
	ret = clk_get_by_index(bus, 0, &priv->icclk);
	if (ret)
		return ret;
	ret = clk_set_rate(&priv->icclk, 24000000);
	if (ret < 0) {
		debug("set clk %ld rate failed\n", priv->icclk.id);
		return ret;
	}
	ret = clk_enable(&priv->icclk);
	if (ret && ret != -ENOSYS && ret != -ENOTSUPP) {
		clk_free(&priv->icclk);
		dev_err(bus, "failed to enable clock\n");
		return ret;
	}
	ret = clk_get_by_index(bus, 1, &priv->pclk);
	if (ret)
		return ret;
