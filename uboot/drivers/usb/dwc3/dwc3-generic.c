...
#if !CONFIG_IS_ENABLED(ARCH_TS_TX536)
...
void dwc3_ts_select_dr_mode(struct udevice *dev, int index,
				enum usb_dr_mode mode)
{
	void *reg_addr;
	u32 val;
	...
	/* 1.cfg clks */
	reg_addr = (void *)0xF0D9A0B0; /* USB3_BUS_CLK_CFG */
	/* set usb3c_bus_clk to 125MHz */
	writel(0x1B, reg_addr);

	reg_addr = (void *)0xF0D9A0AC; /* USB3_FUNC_CLK_CFG */
	/* select 125MHz for usb_ctl_refclk, select 24MHz for usb_phy_refclk */
	writel(0x3A, reg_addr);

	/* 2.cfg usbphy */
	/* set grf_usb3_bus_filter_bypass = 0xf */
	reg_addr = (void *)0xF128C0A0; /* GRF_USB3C_CFG0 */
	writel(0xA9CF, reg_addr);

	/* set grf_usb3_ref_ssp_en = 0x1, grf_usb3_phy_rst = 0x0 */
	reg_addr = (void *)0xF128C0AC; /* GRF_USB3C_CFG3 */
	writel(0x2A04, reg_addr);

	/* 3.cfg GPIOA19 USB_DRVVBUS as output */
	/* set GPIOA19 out val */
	reg_addr = (void *)0xF0D94000; /* GPIO_DATA_REG_OFF */
	val = readl(reg_addr);
	...
	/* set GPIOA19 dir to output */
	reg_addr = (void *)0xF0D94004; /* GPIO_DIR_REG_OFF */
	val = readl(reg_addr);
	val |= BIT(19);
	writel(val, reg_addr);

	/* set UART2_CTSN function to gpio */
	reg_addr = (void *)0xF0D9504C; /* UART2_CTSN_IOCFG */
	writel(0x1043, reg_addr);
}
...
struct dwc3_glue_ops ts_ops = {
	.select_dr_mode = dwc3_ts_select_dr_mode,
};
...
static const struct udevice_id dwc3_glue_ids[] = {
	...
	{ .compatible = "ts,tx536-dwc3", .data = (ulong)&ts_ops },
	{ }
};
