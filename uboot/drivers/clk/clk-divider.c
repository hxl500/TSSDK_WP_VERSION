...
static ulong clk_divider_set_rate(struct clk *clk, unsigned long rate)
{
	...
	debug("\n value [0x%x] flags [0x%x] rate [%ld] parent_rate [%ld]\n", value, divider->flags, rate, parent_rate);
...
#if CONFIG_IS_ENABLED(CLK_TS)
static struct clk *_register_divider_with_sync(struct device *dev, const char *name,
		const char *parent_name, unsigned long flags,
		void __iomem *reg, u8 shift, u8 width, u8 we, u8 sync,
		u8 clk_divider_flags, const struct clk_div_table *table)
{
	struct clk_divider *div;
	struct clk *clk;
...
struct clk *clk_register_divider_with_sync(struct device *dev, const char *name,
		const char *parent_name, unsigned long flags,
		void __iomem *reg, u8 shift, u8 width, u8 we, u8 sync,
		u8 clk_divider_flags)
{
	...
	clk =  _register_divider_with_sync(dev, name, parent_name, flags, reg, shift,
				 width, we, sync, clk_divider_flags, NULL);
	if (IS_ERR(clk))
		return ERR_CAST(clk);
	return clk;
}
