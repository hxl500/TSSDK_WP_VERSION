...
//#define CONFIG_CQSPI_REF_CLK     20000000
...
static int spi_calibration(struct udevice *bus, uint hz)
{
	...
	printf("SF: %dMHz Read data capture delay calibrated to %i (%i - %i)\n",
	      hz / 1000000, (range_hi + range_lo) / 2, range_lo, range_hi);
...
static int cadence_spi_clk_parse(struct udevice *bus)
{
    struct cadence_spi_priv *priv = dev_get_priv(bus);
    struct clk hclk, pclk;

    struct clk ref_clk_div;
    // struct clk ref_clk_gate;
    struct clk ref_clk_mux;
    u32 ref_clk_rate;
	int ret;

	ret = clk_get_by_index(bus, 1, &hclk);
	if (ret) {
		dev_err(bus, "failed to get hclock\n");
		return ret;
	}
	...
	if (ret) {
		clk_free(&hclk);
		dev_err(bus, "failed to enable hclock\n");
		return ret;
	}

	ret = clk_get_by_index(bus, 2, &pclk);
	if (ret) {
		dev_err(bus, "failed to get pclock\n");
		return ret;
	}
	...
	if (ret) {
		clk_free(&pclk);
		dev_err(bus, "failed to enable pclock\n");
		return ret;
	}

	ret = clk_get_by_index(bus, 4, &ref_clk_div);   //for ref clk set parent
	if (ret) {
		dev_err(bus, "failed to get hclock\n");
		return ret;
	}
	...
	if (priv->ref_clk_hz == 0) {
		ret = clk_get_by_index(bus, 0, &ref_clk_mux);               //ref clk mux
        // printf("got ref_clk_mux with ret:%d\n", ret);
		if (ret) {
            ...
            ret = clk_set_parent(&ref_clk_mux, &ref_clk_div);
            if(ret) {
                clk_free(&ref_clk_div);
                dev_err(bus, "failed to enable ref_clk\n");
                return ret;                
            }
            // printf("qspi set parent ok\n");

            if(!dev_read_u32(bus,"ref-clk-rate" , &ref_clk_rate)){  
                //---> specific "ref-clk-rate = <250000000>;" in dts
                ret = clk_set_rate(&ref_clk_div, ref_clk_rate);
                if (IS_ERR_VALUE(ret))
                    return ret;   

                priv->ref_clk_hz = clk_get_rate(&ref_clk_div);
                printf("qspi got ref clk :%u\n", ref_clk_rate);
            } else {                                           
                //---> specific "assigned-clock-rates= <250000000>;" in dts     
                priv->ref_clk_hz = clk_get_rate(&ref_clk_div);
                printf("using default ref clk :%u\n", ref_clk_rate);
            }
            
            ret = clk_enable(&ref_clk_mux);
			if (ret) {
				debug("clk %ld %s enable error\n", ref_clk_mux.id, ref_clk_mux.dev->name);
			    clk_free(&ref_clk_mux);
			}
		}
	}
    return 0;
...
static void wakeup_gpio_init(struct udevice *dev)
{
	struct gpio_desc desc;
	int ret;

	ret = gpio_request_by_name_nodev(dev->node, "wakeup-gpios", 0, &desc, GPIOD_IS_OUT | GPIOD_IS_OUT_ACTIVE);
	if (ret) {
		//pr_err("gpio_request_by_name_nodev not provided %d\n", ret);
	}
...
{
	struct cadence_spi_plat *plat = dev_get_platdata(bus);
	struct cadence_spi_priv *priv = dev_get_priv(bus);
	int ret;
	...
	if (priv->ref_clk_hz == 0) {
		...
		if (ret) {
			...
			debug("clk %ld %s rate %d\n", clk.id, clk.dev->name, priv->ref_clk_hz);
	...
	priv->resets = malloc(sizeof(struct reset_ctl_bulk));
	if (!priv->resets)
		return -ENOMEM;
				   ...
				   const struct spi_mem_op *op)
{
	...
	debug(" cmd:0x%02x bw 0x%x\n",
		 op->cmd.opcode, op->cmd.buswidth);

	if (op->addr.nbytes > 0) {
		debug(" addr:0x%06llx num 0x%x bw 0x%x\n",
			op->addr.val, op->addr.nbytes, op->addr.buswidth);
	}

	if (op->dummy.nbytes > 0) {
		debug(" dummy: num 0x%x bw 0x%x\n",
			  op->dummy.nbytes, op->dummy.buswidth);
	}

	if (op->data.dir == SPI_MEM_DATA_OUT) {
		debug(" data: 0x%08x  num 0x%x bw 0x%x\n",
			  op->data.buf.in ? *(uint32_t *)op->data.buf.in : 0,  op->data.nbytes, op->data.buswidth);
	}
...
static int cadence_spi_of_to_plat(struct udevice *bus)
{
	...
	if (plat->ahbsize >= SZ_8M) {
		priv->use_dac_mode = false;	/* use indirect mode */
...
U_BOOT_DRIVER(cadence_spi) = {
	...
	.ops = &cadence_spi_ops,
	.ofdata_to_platdata = cadence_spi_of_to_plat,
	.platdata_auto_alloc_size = sizeof(struct cadence_spi_plat),
