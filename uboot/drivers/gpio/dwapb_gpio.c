...
static int gpio_dwapb_clk_enable(struct udevice *dev)
{
	int ret;
	struct gpio_dwapb_priv *priv = dev_get_priv(dev);
	...
	if (ret) {
		dev_err(dev, "failed to get gpio clock\n");
		return ret;
	}
	...
	if (ret && ret != -ENOSYS && ret != -ENOTSUPP) {
		clk_free(&priv->clk);
		dev_err(dev, "failed to enable gpio clock\n");
		return ret;
	}
#endif
