...
#if 1
extern rtk_api_ret_t dal_rtl8367c_port_phyEnable_set(rtk_port_t port, rtk_enable_t enable);
extern rtk_api_ret_t dal_rtl8367c_port_phyEnable_get(rtk_port_t port, rtk_enable_t *pEnable);
rtk_api_ret_t rtk_port_phyEnable_set(rtk_port_t port, rtk_enable_t enable)
{
    rtk_api_ret_t retVal;

    RTK_API_LOCK();
    retVal = dal_rtl8367c_port_phyEnable_set(port, enable);
    RTK_API_UNLOCK();

    return retVal;
}
rtk_api_ret_t rtk_port_phyEnable_get(rtk_port_t port, rtk_enable_t *pEnable)
{
    rtk_api_ret_t retVal;

    RTK_API_LOCK();
    retVal = dal_rtl8367c_port_phyEnable_get(port, pEnable);
    RTK_API_UNLOCK();
