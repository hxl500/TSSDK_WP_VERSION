...
#define GRF_BASE	0xF128C000
#define SGRF_BASE	0xF128D000
#define SGRF_SMMU_CFG1                0x54
#define SGRF_SMMU_CFG1_GMAC_LP_SHF     16
#define SGRF_SMMU_CFG1_GMAC_LP_MSK     (1 << SGRF_SMMU_CFG1_GMAC_LP_SHF)
#define RGF_SMMU_STATUS               0xf0
#define RGF_SMMU_STATUS_GMAC_LP_SHF    16
#define RGF_SMMU_STATUS_GMAC_LP_MSK    (1 << SGRF_SMMU_STATUS_GMAC_LP_SHF)
#endif
/* Core registers */
...
static int eqos_start_clks_ts(struct udevice *dev)
{
	...
	mode = eqos->config->interface(dev);
	if (mode == PHY_INTERFACE_MODE_NONE) {
		pr_err("Invalid PHY interface\n");
		return -EINVAL;
	}

	ret = dev_eth_start_clks_ts(eqos->dev_id, mode);

	return ret;
}


static void eqos_stop_clks_ts(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
	...
	dev_eth_stop_clks_ts(eqos->dev_id, mode);
...
static int eqos_start_resets_ts(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
	int ret;
	struct reset_ctl_bulk rst_bulk;
    
	#if defined(CONFIG_ARCH_TS_TX536) //defined(CONFIG_ARCH_TS_TX536)
	uint32_t value;
	int timeout = 100; //us
	#endif
	...
	if (ret < 0) {
		pr_err("reset_get_bulk(eqos_reset, get) failed: %d", ret);
		return ret;
	}
#if defined(CONFIG_ARCH_TS_TX536)
	value = readl(SGRF_BASE + SGRF_SMMU_CFG1);
	value |= SGRF_SMMU_CFG1_GMAC_LP_MSK;
	writel(value, SGRF_BASE + SGRF_SMMU_CFG1);

	do {
		value = readl(GRF_BASE + RGF_SMMU_STATUS);
		if (timeout <= 0) {
			pr_err("%s: GMAC wait smmu lp time out.\n", __func__);
		}
		udelay(10);
		timeout -= 10;
	} while (!((value >> RGF_SMMU_STATUS_GMAC_LP_SHF) & 0x01));
#endif
	ret = reset_assert_bulk(&rst_bulk);
	if (ret < 0) {
		pr_err("reset_assert_bulk() failed: %d", ret);
		return ret;
	}

	udelay(2);

	ret = reset_deassert_bulk(&rst_bulk);
	if (ret < 0) {
		pr_err("reset_deassert_bulk() failed: %d", ret);
		return ret;
	}
#if defined(CONFIG_ARCH_TS_TX536)
	value = readl(SGRF_BASE + SGRF_SMMU_CFG1);
...
static int eqos_stop_resets_ts(struct udevice *dev)
{
#if 0
	struct eqos_priv *eqos = dev_get_priv(dev);
	int ret;
...
static ulong eqos_get_tick_clk_rate_ts(struct udevice *dev)
{
	return 150 * 1000000;
}

static int eqos_calibrate_pads_ts(struct udevice *dev)
{
	return 0;
}

static int eqos_disable_calibration_ts(struct udevice *dev)
{
	return 0;
}
...
static int eqos_set_tx_clk_speed_ts(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
	ulong rate;
	int ret;
	...
	ret = dev_eth_txclk_set_rate(eqos->dev_id, rate);
	if (ret < 0) {
		pr_err("ts (tx_clk, %lu) failed: %d", rate, ret);
		return ret;
	}
...
static int eqos_adjust_link_without_phy(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
	int ret;
	bool en_calibration;
	...
	printf("call eqos_set_tx_clk_speed()\n");
	ret = eqos->config->ops->eqos_set_tx_clk_speed(dev);
	if (ret < 0) {
		pr_err("eqos_set_tx_clk_speed() failed: %d", ret);
		return ret;
...
#endif
		debug("addr %d  eqos->max_speed %d\r\n", addr, eqos->max_speed);
		eqos->phy = phy_connect(eqos->mii, addr, dev,
					eqos->config->interface(dev));
		if (!eqos->phy) {
			pr_err("phy_connect() failed.\n");
			ret = eqos_adjust_link_without_phy(dev);
			if (ret < 0) {
				pr_err("eqos_adjust_link() failed: %d", ret);
				goto err_shutdown_phy;
			}
			pr_err("goto eth_after_phy_init\n");
			goto eth_after_phy_init;
		}
...
static int eqos_probe_resources_ts(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
	int ret;
	phy_interface_t mode;
	...
	eqos->dev_id = eqos_get_dev_id(dev->name);

	debug("%s(dev id =%d):\r\n", __func__, eqos->dev_id);
	...
	ret = dev_eth_interface_init(eqos->dev_id, mode);
	if (ret)
		return -EINVAL;
...
static phy_interface_t eqos_get_interface_ts(struct udevice *dev)
{
	const char *phy_mode;
	phy_interface_t mode = PHY_INTERFACE_MODE_NONE;

	debug("%s(dev=%p): %s--gd %p offset %d\n", __func__, dev, dev->name,gd->fdt_blob,dev_of_offset(dev));
...
static int eqos_remove_resources_ts(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
...
extern void eth_switch_set_mdio_dev(struct mii_dev *dev);
...
#if defined CONFIG_RTL8367RBVB_SWITCH
	eth_switch_set_mdio_dev(eqos->mii);
#endif
...
static struct eqos_ops eqos_ts_ops = {
	.eqos_inval_desc = eqos_inval_desc_generic,
	.eqos_flush_desc = eqos_flush_desc_generic,
	.eqos_inval_buffer = eqos_inval_buffer_generic,
	.eqos_flush_buffer = eqos_flush_buffer_generic,
	.eqos_probe_resources = eqos_probe_resources_ts,
	.eqos_remove_resources = eqos_remove_resources_ts,
	.eqos_stop_resets = eqos_stop_resets_ts,
	.eqos_start_resets = eqos_start_resets_ts,
	.eqos_stop_clks = eqos_stop_clks_ts,
	.eqos_start_clks = eqos_start_clks_ts,
	.eqos_calibrate_pads = eqos_calibrate_pads_ts,
	.eqos_disable_calibration = eqos_disable_calibration_ts,
	.eqos_set_tx_clk_speed = eqos_set_tx_clk_speed_ts,
	.eqos_get_tick_clk_rate = eqos_get_tick_clk_rate_ts
};

static const struct eqos_config __maybe_unused eqos_ts_config = {
	...
	.interface = eqos_get_interface_ts,
	.ops = &eqos_ts_ops
};

static const struct udevice_id eqos_ids[] = {
	{
		.compatible = "snps,ts-eqos",
		.data = (ulong)&eqos_ts_config
	},
