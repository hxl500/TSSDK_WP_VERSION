...
static int ts_tx5215_gpio_func_sel(struct udevice *dev, unsigned pin, 
...
*/
static const char * const ts_tx5215_pins[PINMAX] = {

};

static const char * const ts_tx5215_pins_muxing[PINMAX] = {
...
static const struct pinconf_param ts_tx5215_conf_params[] = {
...
static unsigned int ts_tx5215_pins_param[ARRAY_SIZE(ts_tx5215_pins)];
static unsigned int ts_tx5215_pins_value[ARRAY_SIZE(ts_tx5215_pins)];

static int ts_tx5215_get_pins_count(struct udevice *dev)
{
	return ARRAY_SIZE(ts_tx5215_pins);
}

static const char *ts_tx5215_get_pin_name(struct udevice *dev, unsigned selector)
{
	return ts_tx5215_pins[selector];
}

static int ts_tx5215_get_pin_muxing(struct udevice *dev,
				  ...
				  char *buf, int size)
{
	...
	snprintf(buf, size, "%s", ts_tx5215_pins_muxing[selector]);

	if (ts_tx5215_pins_param[selector]) {
		for (i = 0, p = ts_tx5215_conf_params;
		     i < ARRAY_SIZE(ts_tx5215_conf_params);
		     i++, p++) {
			if ((ts_tx5215_pins_param[selector] & BIT(p->param)) &&
			    (!!(ts_tx5215_pins_value[selector] & BIT(p->param)) ==
			     p->default_value)) {
				strncat(buf, " ", size);
...
static int ts_tx5215_get_groups_count(struct udevice *dev)
{
	struct ts_pinctrl *pctrl = (struct ts_pinctrl *)dev_get_driver_data(dev);
	return pctrl->ngroups;
}

static const char *ts_tx5215_get_group_name(struct udevice *dev,
					  unsigned selector)
{
	struct ts_pinctrl *pctrl = (struct ts_pinctrl *)dev_get_driver_data(dev);
...
static int ts_tx5215_get_functions_count(struct udevice *dev)
{
	struct ts_pinctrl *pctrl = (struct ts_pinctrl *)dev_get_driver_data(dev);
	return pctrl->nfuncs;
}

static const char *ts_tx5215_get_function_name(struct udevice *dev,
					     unsigned selector)
{
	struct ts_pinctrl *pctrl = (struct ts_pinctrl *)dev_get_driver_data(dev);
...
static int ts_tx5215_pinmux_set(struct udevice *dev, unsigned pin_selector,
			      unsigned func_selector)
{
    int ret;
	debug("ts_tx5215 pinmux: pin = %d (%s), function = %d (%s)\n",
	      pin_selector, ts_tx5215_get_pin_name(dev, pin_selector),
	      func_selector, ts_tx5215_get_function_name(dev, func_selector));

	ts_tx5215_pins_param[pin_selector] = 0;
	ts_tx5215_pins_value[pin_selector] = 0;

    ret = ts_tx5215_gpio_func_sel(dev, pin_selector, func_selector);
	return ret;
}
...
static int ts_tx5215_pinmux_group_set(struct udevice *dev,
				    ...
				    unsigned func_selector)
{
	...
	for (u32 i = 0; i < pgrp->npins; i++) {
		ts_tx5215_gpio_func_sel(dev, pgrp->pins[i], pgrp->mux_val);
	}
	return 0;
}

static int ts_tx5215_pinconf_set(struct udevice *dev, unsigned pin_selector,
			       unsigned param, unsigned argument)
{
	debug("ts_tx5215 pinconf: pin = %d (%s), param = %d, arg = %d\n",
	      pin_selector, ts_tx5215_get_pin_name(dev, pin_selector),
	      param, argument);

	ts_tx5215_pins_param[pin_selector] |= BIT(param);
	if (argument)
		ts_tx5215_pins_value[pin_selector] |= BIT(param);
	else
		ts_tx5215_pins_value[pin_selector] &= ~BIT(param);
...
static int ts_tx5215_pinconf_group_set(struct udevice *dev,
				     unsigned group_selector,
				     unsigned param, unsigned argument)
{
	debug("ts_tx5215 pinconf: group = %d (%s), param = %d, arg = %d\n",
	      group_selector, ts_tx5215_get_group_name(dev, group_selector),
	      param, argument);

	return 0;
}

const struct pinctrl_ops ts_tx5215_pinctrl_ops = {
	.get_pins_count = ts_tx5215_get_pins_count,
	.get_pin_name = ts_tx5215_get_pin_name,
	.get_pin_muxing = ts_tx5215_get_pin_muxing,
	.get_groups_count = ts_tx5215_get_groups_count,
	.get_group_name = ts_tx5215_get_group_name,
	.get_functions_count = ts_tx5215_get_functions_count,
	.get_function_name = ts_tx5215_get_function_name,
	.pinmux_set = ts_tx5215_pinmux_set,
	.pinmux_group_set = ts_tx5215_pinmux_group_set,
	.pinconf_num_params = ARRAY_SIZE(ts_tx5215_conf_params),
	.pinconf_params = ts_tx5215_conf_params,
	.pinconf_set = ts_tx5215_pinconf_set,
	.pinconf_group_set = ts_tx5215_pinconf_group_set,
				...
				struct ts_pinctrl *pctrl)
{
	...
	debug("ts_pinctrl_parse_dt done\n");
	return 0;
}
...
static int ts_pinctrl_probe(struct udevice *dev)
{
	...
	dev_dbg(dev, "ts pinctrl initialized\n");
	ts_pinctrl_init_flag = true;

	return 0;
}

static const struct udevice_id ts_tx5215_pinctrl_match[] = {
	{ .compatible = "ts_tx5215,pinctrl" },
	{ /* sentinel */ }
};

U_BOOT_DRIVER(ts_tx5215_pinctrl) = {
	.name = "ts_tx5215_pinctrl",
	.id = UCLASS_PINCTRL,
	.of_match = ts_tx5215_pinctrl_match,
	.ops = &ts_tx5215_pinctrl_ops,
