...
int spinand_select_target(struct spinand_device *spinand, unsigned int target)
{
	...
	if (WARN_ON(target >= nand->memorg.ntargets)){
		printf("Size out of range, target=%d ntargets=%d\n", target, nand->memorg.ntargets);
		//spinand->cur_target = nand->memorg.ntargets - 1;
		return -EINVAL;
		//return 0;
	}
...
#ifdef CONFIG_NAND_BOOT //CONFIG_MTD_SPI_NAND
...
void board_spinand_init(void)
{
	...
	ret = uclass_get_device_by_driver(UCLASS_MTD,
					  ...
					  &g_dev);
	if (ret && ret != -ENODEV)
		pr_err("Failed to initialize SPI NAND controller. (error %d)\n",
		       ret);
	mtd = dev_get_uclass_priv(g_dev);
	nand_register(0, mtd);
}

#ifndef CONFIG_SYS_NAND_SELF_INIT
static int nand_init_chip(int i)
{
	struct spinand_device *spinand = dev_get_priv(g_dev);
	struct spi_slave *slave = dev_get_parent_priv(g_dev);
	struct mtd_info *mtd = dev_get_uclass_priv(g_dev);
	struct nand_device *nand = spinand_to_nand(spinand);
	int ret;
	...
	sprintf(mtd->name, "spi-nand%d", spi_nand_idx++);
	spinand->slave = slave;
	spinand_set_of_node(spinand, g_dev->node.np);
#endif
