...
#ifndef __TSINGMICRO_TX5215DV300_EVB132_H
#define __TSINGMICRO_TX5215DV300_EVB132_H
...
#if 1
/* MTD device partition table for spinor*/
#define TS_FIP_1		0x000000	//0
#define TS_HW_INFO		0x07c000	//496K
#define TS_KERNEL_1		0x080000	//512K
#define TS_FIP_MTD_SIZE	TS_KERNEL_1 //
#define TS_ROOTFS_1		0x480000	//4.5M
#define TS_KERNEL_MTD_SIZE SZ_4M
#define TS_RAMDISK_1	TS_ROOTFS_1
#define TS_USER_1		0x780000	//7.5M
#define TS_ROOTFS_MTD_SIZE 0x300000
#define TS_USER_2		0xe80000	//14.5M
#else
/* MTD device partition table for spinand*/
#define TS_FIP_1		0x000000	//0
#define TS_HW_INFO		0x080000	//512K
#define TS_FIP_MTD_SIZE	TS_HW_INFO //
#define TS_KERNEL_1		0x080000	//512K
#define TS_ROOTFS_1		0x580000	//5.5M
#define TS_KERNEL_MTD_SIZE 0x500000
#define TS_RAMDISK_1	TS_ROOTFS_1
#define TS_USER_1		0xa80000	//10.5M
#define TS_ROOTFS_MTD_SIZE 0x500000
#define TS_USER_2		0xe80000	//14.5M
#endif
...
#define CONFIG_EXTRA_ENV_SETTINGS					\
		"boardname=TS-TX5215DV300-EVB132\0" \
		"fip_name=fip_tx5215dv300_evb132-emmc.bin\0" \
		"kernel_name=ts-tx5215dv300-evb132.itb\0"  \
		"kernel_ramfs_name=ts-tx5215dv300-evb132-ramdisk.itb\0" \
		...
		"rootfs_name_mtd=rootfs.squashfs\0"  \
		"usrfs_name_mtd=usr.squashfs\0"  \
		"rootfs_ubi_name=rootfs.ubi\0"  \
		"loadaddr=0x00a00000\0" \
		"loadaddr_gz=0x05a00000\0" \
		"netdev=eth0\0"			\
		"mmcdev=0\0"			\
		...
		"mtdparts=mtdparts=f1000000.spi.0:512K(boot),4M(kernel),3M(romfs)ro,7M(user),1528K(root),-(config) \0"\
		"ramroot=root=/dev/ram0 rootfstype=ext4 rootwait rw \0"\
		"mtdroot=ubi.mtd=2,2048 root=ubi0:rootfs rootfstype=ubifs rw  mtdparts=f1000000.spi.0:6M(boot),16M(kernel),16M(kernel2)ro,109M(ubi),-(ubi2)\0"\
		"norroot=root=/dev/mtdblock2 rootfstype=squashfs init=/sbin/init rootwait\0"\
		...
		"read_kernel_spinand=mtd read ${spinand_name} ${loadaddr} ${kernel_mtd} ${kernel_mtd_size}\0" \
...
" __stringify(TS_MMC_ROOTFS)" " __stringify(TS_MMC_UPDATE) " \0"      \
		...
		"write_rootfs_spinand=mtd erase ${spinand_name} ${rootfs_mtd} ${rootfs_mtd_size}; mtd write ${spinand_name} ${loadaddr} ${rootfs_mtd} ${rootfs_mtd_size}\0" \
		...
		"update_rootfs_spinand=run nfsargs; run switch_spinand; while true; do if tftp ${loadaddr} ${rootfs_ubi_name}; then run write_rootfs_spinand; exit; else echo [TFTP]:FAILED ROOTFS!; fi;  done;\0" \
		"update_usrfs_spinor=run nfsargs; while true; do  if tftp ${loadaddr} ${usrfs_name_mtd}; then run switch_spinor; run write_usrfs_spinor; exit; else echo [TFTP]:FAILED USRFS!; fi; done; \0" \
		...
		"mtd_bootargs=setenv bootargs ${arg_console} ${mtdroot} ${hw_args}\0" \
		...
		"boot_spinand=run switch_spinand; run mtd_bootargs; run read_kernel_spinand; run bootm_kernel\0" \
