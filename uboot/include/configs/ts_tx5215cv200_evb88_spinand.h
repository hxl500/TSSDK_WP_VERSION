...
#define TS_ROOTFS_2			0x2000000	//32.0M
#define TS_ROOTFS_MTD_SIZE	0x1400000
#define TS_ROOT				0x3400000	//52.0M
#define TS_MTD_UPDATE		0x4800000   //71.0M
...
#define CONFIG_EXTRA_ENV_SETTINGS					\
		"boardname=TS-TX5215CV200-EVB88-SPINAND\0" \
		"fip_name=fip_tx5215cv200_evb88spinand.bin\0" \
		"kernel_name=ts-tx5215cv200-evb88-spinand.itb\0"  \
		...
		"arg_console=console=ttyS0,115200n8 quiet lpj=" __stringify(TS_LPJ) "\0"\
		...
		"spinandparts=mtdparts=spi0.0:896K(boot1),896K(boot2),256K(env),5M(kernel1),5M(kernel2),20M(rootfs1),20M(rootfs2),20M(root),55M(update),-(config) \0"\
		...
		"read_kernel_spinand=nand read ${loadaddr} ${kernel_mtd} ${kernel_mtd_size}\0" \
