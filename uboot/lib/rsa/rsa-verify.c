...
static int rsa_mod_exp_hw(struct key_prop *prop, const uint8_t *sig,
				const uint32_t sig_len, const uint32_t key_len, uint8_t *output)
{
	int ret;
	uint32_t eBitLen = 17;
	uint32_t e[2], n[64], sig_convert[64], sig_reverse[64];;

	if (key_len != RSA2048_BYTES)
		return -EINVAL;

	rsa_convert_big_endian(e, (uint32_t *)prop->public_exponent, sizeof(e) / sizeof(uint32_t));
	rsa_convert_big_endian(n, (uint32_t *)prop->modulus, sizeof(n) / sizeof(uint32_t));
	rsa_convert_big_endian(sig_convert, (uint32_t *)sig, sizeof(sig_convert) / sizeof(uint32_t));

	ret = RSA_ModExp(sig_convert, e, n, sig_reverse, eBitLen, prop->num_bits);
	if (RSA_SUCCESS != ret)
		return ret;
	rsa_convert_big_endian((uint32_t *)output, sig_reverse, sizeof(sig_reverse) / sizeof(uint32_t));
