...
{
  #include "tuning_sc2336p_linear_dual_wl.h"
};

static BOOL g_isJsonLoaded = FALSE;

static void load_sc2336p_normal_tuning_json(const char8 *filename)
{
  if (ISP_RET_SUCCESS == Json_To_Params_From_File(filename, JSON_TUNING, (void*)&g_tuningParams))
    g_isJsonLoaded = TRUE;
}

tune_params_t *load_sc2336p_normal_wl_tuning(const char8 *filename)
{
  FW_FUNC_ENTER();

  if (!g_isJsonLoaded)
    load_sc2336p_normal_tuning_json(filename);
