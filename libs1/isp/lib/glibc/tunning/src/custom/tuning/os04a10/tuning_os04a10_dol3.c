...
static tune_params_t g_tuningParams =
{
    ...
    #include "tuning_imx415_dol3_oct.h"
    #else
    #include "tuning_imx415_dol3_tit.h"
    #endif
};
...
static void Load_Imx415_Dol3_Tuning_JSON(const char8 *filename)
{
    if (ISP_RET_SUCCESS == Json_To_Params_From_File(filename, JSON_TUNING, (void*)&g_tuningParams))
    {
        ...
        #ifdef PC_SIMU
        Print_Struct(&g_tuningParams, PRINT_TYPE_TUNING, "415_dol3.h");
        #endif
    }
}

/*
* Load_Imx415_Dol3_Tuning
*
...
*
*/
tune_params_t* Load_Imx415_Dol3_Tuning(const char8 *filename)
{
    FW_FUNC_ENTER();

    if (!g_isJsonLoaded)
    {
        Load_Imx415_Dol3_Tuning_JSON(filename);
    }
