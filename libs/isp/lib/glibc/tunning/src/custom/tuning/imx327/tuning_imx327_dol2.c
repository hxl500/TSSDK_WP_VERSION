...
static tune_params_t g_tuningParams =
{
    ...
    #include "tuning_imx327_dol2_oct.h"
    #else
    #include "tuning_imx327_dol2_tit.h"
    #endif
};
...
static void Load_Imx327_Dol2_Tuning_JSON(const char8 *filename)
{
    if (ISP_RET_SUCCESS == Json_To_Params_From_File(filename, JSON_TUNING, (void*)&g_tuningParams))
    {
        ...
        #ifdef PC_SIMU
        Print_Struct(&g_tuningParams, PRINT_TYPE_TUNING, "327_dol2.h");
        #endif
    }
    else
    {
...
*
*/
tune_params_t* Load_Imx327_Dol2_Tuning(const char8 *filename)
{
    FW_FUNC_ENTER();

    if (!g_isJsonLoaded)
    {
        Load_Imx327_Dol2_Tuning_JSON(filename);
    }
