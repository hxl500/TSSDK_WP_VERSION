...
#include "sc2337p_sensor.h"

static int oldframelength = 2622;


static isp_ret_t SC2337P_Normal_Fill_Exposure_Reg(sensor_apply_exp_param_t *pExpParam, drv_regs_array_info_t *pRegInfo)
{
    isp_ret_t ret = ISP_RET_SUCCESS;
    // According to sc2337p datasheet
   ...
   // if (oldframelength != pExpParam->frameLengthLines)
	...
	{
	   // printf("cam:%p,FLL:%d  ELL:%d  \n",pExpParam, pExpParam->frameLengthLines,pExpParam->expLineLong);
	   pExpParam->expLineLong = pExpParam->frameLengthLines - 6;
	} 
...
isp_ret_t SC2337P_Fill_Exposure_Reg(sensor_apply_exp_param_t *pExpParam, drv_regs_array_info_t *pRegInfo)
{
    ...
    if (ret == ISP_RET_SUCCESS)
    {
        ...
        {
            ret = SC2337P_Normal_Fill_Exposure_Reg(pExpParam, pRegInfo);
        }
        else
        {
