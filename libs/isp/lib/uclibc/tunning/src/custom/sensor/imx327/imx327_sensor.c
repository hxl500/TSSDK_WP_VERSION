...
#define IMX327_DOL2_SHORT_1_RHS (509)

#define IMX327_DOL3_SHORT_1_RHS (805)   /// Mid frame max line count, should satisfy 3*n + 7
#define IMX327_DOL3_SHORT_2_RHS (1112)  /// M +S Max line count, should satisfy 3*n + 14

static u32 IMX327_Reg_Gain_Table[256] =
{
...
static u32 IMX327_Gain_To_Reg(u32 expoAgain)
{

    u32 i = 0;
    u32 gainRegVal = 0x80;

    if (expoAgain <= IMX327_Reg_Gain_Table[0])
    {
        gainRegVal = 0;
    }
    else if (expoAgain >= IMX327_Reg_Gain_Table[255])
    {
        gainRegVal = 255;
    }
    else
    {
        while (i < 8)
        {
            if ((expoAgain >= IMX327_Reg_Gain_Table[gainRegVal - 1])
                && (expoAgain <= IMX327_Reg_Gain_Table[gainRegVal]))
            {
                if (expoAgain != IMX327_Reg_Gain_Table[gainRegVal])
                {
                    gainRegVal = gainRegVal - 1;
                }
                break;
            }
            else if (expoAgain > IMX327_Reg_Gain_Table[gainRegVal])
            {
                gainRegVal += (0x80 >> (i+1));
            }
            else if (expoAgain < IMX327_Reg_Gain_Table[gainRegVal])
...
static u32 IMX327_Reg_To_Gain(u32 regVal)
{
    return IMX327_Reg_Gain_Table[regVal];
}

static isp_ret_t IMX327_DOL3_Fill_Exposure_Reg(sensor_apply_exp_param_t * pExpParam, drv_regs_array_info_t * pRegInfo)
{
    ...
    u32 RHS1RegVal = IMX327_DOL3_SHORT_1_RHS;
    u32 SHS1RegVal = 0;
    ...
    u32 RHS2RegVal = IMX327_DOL3_SHORT_2_RHS;
    u32 SHS2RegVal = 0;
    ...
    if (pExpParam->gainIndividualEn)
    {
        ...
        /// frame id 1
        gainRegValue = IMX327_Gain_To_Reg(pExpParam->analogGain1);
        pRegInfo->settings[pRegInfo->settingSize].regAddr = 0x30F2;
        pRegInfo->settings[pRegInfo->settingSize].regData = gainRegValue & 0xFF;
        pRegInfo->settingSize ++;
        pExpParam->analogGain1 = IMX327_Reg_To_Gain(gainRegValue);

        /// frame id 2
        gainRegValue = IMX327_Gain_To_Reg(pExpParam->analogGain2);
        pRegInfo->settings[pRegInfo->settingSize].regAddr = 0x30F6;
        pRegInfo->settings[pRegInfo->settingSize].regData = gainRegValue & 0xFF;
        pRegInfo->settingSize ++;
        pExpParam->analogGain2 = IMX327_Reg_To_Gain(gainRegValue);
    }
    else
    {
        pRegInfo->settings[pRegInfo->settingSize].regAddr = 0x3010;
...
static isp_ret_t IMX327_DOL2_Fill_Exposure_Reg(sensor_apply_exp_param_t *pExpParam, drv_regs_array_info_t *pRegInfo)
{
    u32 gainRegValue = 0;
    ...
    u32 RHS1RegVal = IMX327_DOL2_SHORT_1_RHS;
    u32 SHS1RegVal = 0;
...
//    SENSOR_INFO(" IMX327_DOL2_Fill_Integration_Time_Reg FrameLength %d  SHS2RegVal %d RHS1RegVal %d SHS1RegVal %d gainVal %d ", pExpParam->frameLengthLines ,
...
static isp_ret_t IMX327_Normal_Fill_Exposure_Reg(sensor_apply_exp_param_t *pExpParam, drv_regs_array_info_t *pRegInfo)
{

    isp_ret_t ret = ISP_RET_SUCCESS;

    /// According to imx327 datasheet
    ///     1. 0x3020/0x3021(SHS1)          means shutter sweep time
    ///     2. 0x301a/0x3019/0x3018(VMAX)   means the number of lines per frame(FrameLength)
    ...
    pRegInfo->settings[pRegInfo->settingSize].regAddr = 0x3021;
    pRegInfo->settings[pRegInfo->settingSize].regData = regLongExpoIntegrationTime >> 8 ;
    pRegInfo->settingSize ++;
...
isp_ret_t IMX327_Fill_Exposure_Reg(sensor_apply_exp_param_t *pExpParam, drv_regs_array_info_t *pRegInfo)
{
    ...
    if (ret == ISP_RET_SUCCESS)
    {
        ...
        {
            ret = IMX327_Normal_Fill_Exposure_Reg(pExpParam, pRegInfo);
        }
        else if (pExpParam->hdrMode == SENSOR_DOL2_MODE)
        {
            ret = IMX327_DOL2_Fill_Exposure_Reg(pExpParam, pRegInfo);
        }
        else if (pExpParam->hdrMode == SENSOR_DOL3_MODE)
        {
            ret = IMX327_DOL3_Fill_Exposure_Reg(pExpParam, pRegInfo);
        }
        else
        {
