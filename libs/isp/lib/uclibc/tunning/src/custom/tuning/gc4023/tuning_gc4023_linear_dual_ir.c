...
{
  #include "tuning_gc4023_linear_dual_ir.h"
};

static BOOL g_isJsonLoaded = FALSE;

static void load_gc4023_normal_tuning_json(const char8 *filename)
{
  if (ISP_RET_SUCCESS == Json_To_Params_From_File(filename, JSON_TUNING, (void*)&g_tuningParams))
    g_isJsonLoaded = TRUE;
}

tune_params_t *load_gc4023_tuning_linear_dual_ir(const char8 *filename)
{
  FW_FUNC_ENTER();

  if (!g_isJsonLoaded)
    load_gc4023_normal_tuning_json(filename);
