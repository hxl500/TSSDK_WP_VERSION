...
static tune_params_t g_tuningParams =
{
    ...
    #include "tuning_imx327_normal_oct.h"

    #elif defined (HW_BUILD_FOR_TITIAN)

    #include "tuning_imx327_normal_tit.h"
...
static void Load_Imx327_Normal_Tuning_JSON(const char8 *filename)
{
    if (ISP_RET_SUCCESS == Json_To_Params_From_File(filename, JSON_TUNING, (void*)&g_tuningParams))
    {
        g_isJsonLoaded = TRUE;
        #ifdef PC_SIMU
        Print_Struct(&g_tuningParams, PRINT_TYPE_TUNING, "327_nor.h");
        #endif
    }
    else
    {
        #ifdef PC_SIMU
        Params_To_Json_File("tuning_nor.json", JSON_TUNING, (void*)&g_tuningParams);
        #endif
    }
}

/*
* Load_Imx327_Normal_Tuning
*
* @brief Load tuning parameters of imx327 from static headers or json
*
...
*
*/
tune_params_t* Load_Imx327_Normal_Tuning(const char8 *filename)
{
    FW_FUNC_ENTER();

    if (!g_isJsonLoaded)
    {
        /* Load_Imx327_Normal_Tuning_JSON(filename); */
    }
