...
{
    #include "calibration_hi556_dual_ir.h"
};

static BOOL g_isJsonLoaded = FALSE;

static void load_hi556_calibration_json(const char *filename)
{
  if (ISP_RET_SUCCESS == Json_To_Params_From_File(filename, JSON_CALIBRATION, (void*)&g_calParams))
    g_isJsonLoaded = TRUE;
}

calibration_params_t *load_hi556_ir_calibration(const char *filename)
{
  FW_FUNC_ENTER();

  if (!g_isJsonLoaded)
    load_hi556_calibration_json(filename);
