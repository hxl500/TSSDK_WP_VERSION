...
static tune_params_t g_tuningParams =
{
    ...
    #include "tuning_ov5647_normal_oct.h"
    #endif
};
...
static void Load_Ov5647_Normal_Tuning_JSON(const char8 *filename)
{
    if (ISP_RET_SUCCESS == Json_To_Params_From_File(filename, JSON_TUNING, (void*)&g_tuningParams))
    {
        ...
        #ifdef PC_SIMU
        Print_Struct(&g_tuningParams, PRINT_TYPE_TUNING, "ov_nor.h");
        #endif
    }
}

/*
* Load_Ov5647_Normal_Tuning
*
* @brief Load tuning parameters of ov5647 from static headers or json
*
...
*
*/
tune_params_t* Load_Ov5647_Normal_Tuning(const char8 *filename)
{
    FW_FUNC_ENTER();

    if (!g_isJsonLoaded)
    {
        Load_Ov5647_Normal_Tuning_JSON(filename);
    }
