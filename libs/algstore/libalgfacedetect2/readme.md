...
- `TS_ALG_FaceDetect2_Init`：初始化人脸检测算法。
- `TS_ALG_FaceDetect2_Process`：执行人脸检测算法。
- `TS_ALG_FaceDetect2_Exit`：释放人脸检测算法资源。
- `TS_ALG_FaceDetect2_SetParam`：设置人脸检测算法参数。
- `TS_ALG_FaceDetect2_GetParam`：获取人脸检测算法参数。
- `TS_ALG_FaceDetect2_GetVersion`：获取人脸检测算法版本信息。

#### `TS_ALG_FaceDetect2_Init`
...
const `TS_CHAR`* `TS_ALG_FaceDetect2_GetVersion`(TS_VOID);
...
- ALG_FACEDET2_BOX_S：定义人脸检测结果框。
- ALG_FACEDET2_PARAM_S：定义人脸检测参数信息。
- ALG_FACEDET2_RESULT_S: 定义人脸检测结果信息。
...
`typedef struct tsALGFACEDET2_BOX_S{`
...
最大支持25个人脸框，即MAX_FACEDET2_NUM = 25。
