...
static struct snd_soc_codec *es7243e_codec;
...
static int es7243e_probe(struct snd_soc_codec *codec)
{
	...
	for (index = 0; index < (ES7243E_CHANNELS_MAX) / 2; index++) {
		...
		es7243e_write(ES7243E_PGA1, (0x10 | pga_gain[index * 2]),
			      i2c_clt[index]);
		es7243e_write(ES7243E_PGA2, (0x10 | pga_gain[index * 2 + 1]),
			      i2c_clt[index]);
...
es7243e_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *i2c_id)
{
	...
	es7243e->regmap = devm_regmap_init_i2c(i2c, &es7243e_regmap_config);
...
MODULE_DESCRIPTION("ASoC es7243e driver");
