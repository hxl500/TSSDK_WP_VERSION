...
/* codec is master*/
static const struct _coeff_div master_coeff_div[] = {
	/* 8k */
	...
	{8192000, 8000, 1024, 0x7, 0x0, 0x1b},
...
/* codec is salve	*/
static const struct _coeff_div slave_coeff_div[] = {
	/* 8k */
	{12288000, 8000, 256, 0x2, 0x0},
	{12000000, 8000, 256, 0x2, 0x0},
	{8192000, 8000, 1024, 0x7, 0x0},
...
static inline int get_coeff(struct es8388_priv *es8388, int rate)
{
	int i;
	unsigned int mclk = es8388->sysclk;

	if (es8388->is_master) {
		for (i = 0; i < ARRAY_SIZE(master_coeff_div); i++) {
			if ((master_coeff_div[i].rate == rate)
				&& (master_coeff_div[i].mclk == mclk))
				return i;
		}
	} else {
		for (i = 0; i < ARRAY_SIZE(slave_coeff_div); i++) {
			if ((slave_coeff_div[i].rate == rate)
				&& (slave_coeff_div[i].mclk == mclk))
				return i;
		}
	}
...
static unsigned int rates_8192000[] = {
	8000, 16000, 32000,
};

static struct snd_pcm_hw_constraint_list constraints_8192000 = {
	.count = ARRAY_SIZE(rates_8192000),
	.list = rates_8192000,
};
				 ...
				 int clk_id, unsigned int freq, int dir)
{
	...
	switch (freq) {
	case 8192000:
		es8388->sysclk_constraints = &constraints_8192000;
		es8388->sysclk = freq;
		return 0;
	case 11289600:
	case 18432000:
...
static int es8388_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_CBM_CFM:	/* MASTER MODE */
		...
		es8388->is_master = true;
		break;
	case SND_SOC_DAIFMT_CBS_CFS:	/* SLAVE MODE */
		iface &= ~ES8388_MASTERMODE_MSC;
		es8388->is_master = false;
		break;
	default:
		return -EINVAL;
	}
				...
				struct snd_soc_dai *dai)
{
	...
	iface = snd_soc_read(codec, ES8388_IFACE);
	coeff = get_coeff(es8388, params_rate(params));
	if (coeff < 0) {
		dev_err(codec->dev,
	...
	if (coeff >= 0) {
		...
		if (es8388->is_master) {
			snd_soc_write(codec, ES8388_ADCCONTROL5,
						master_coeff_div[coeff].sr |
						(master_coeff_div[coeff].usb) << 5);
			snd_soc_write(codec, ES8388_DACCONTROL2,
						master_coeff_div[coeff].sr |
						(master_coeff_div[coeff].usb) << 5);
			iface &= ~(ES8388_MASTERMODE_BCLKDIV_MASK);
			iface |= (master_coeff_div[coeff].bclk_div << ES8388_MASTERMODE_BCLKDIV_POS);
			snd_soc_write(codec, ES8388_IFACE, iface);
		} else {
			snd_soc_write(codec, ES8388_ADCCONTROL5,
						slave_coeff_div[coeff].sr |
						(slave_coeff_div[coeff].usb) << 5);
			snd_soc_write(codec, ES8388_DACCONTROL2,
						slave_coeff_div[coeff].sr |
						(slave_coeff_div[coeff].usb) << 5);
		}
...
static int es8388_dai_probe(struct snd_soc_dai *codec_dai)
{
	struct snd_soc_codec *codec = codec_dai->codec;
	int card_number = -1;

	card_number = codec->component.card->card_number;
