...
{
	pr_err("%s alg[%s] self test disable \n", __func__, alg);
	return 0;
}
...
struct trng_test_suite {
	const struct trng_testvec *vecs;
	unsigned int count;
};
...
static void hexdump(const char *buf, unsigned int len)
{
			   ...
			   bool use_digest, const int align_offset)
{
	...
	for (i = 0; i < tcount; i++) {
		...
		if (memcmp(result, template[i].digest,
			...
			hexdump(template[i].plaintext, template[i].psize);
			printk(KERN_ERR "template[%d].plaintext\n",i);
			ret = -EINVAL;
			goto out;
		}
	}
...
static int test_trng(struct crypto_rng *tfm,
			  const struct trng_testvec *template,
			  unsigned int tcount)
{
	const char *algo = crypto_tfm_alg_driver_name(crypto_rng_tfm(tfm));
	int i, err;
	...
	for (i = 0; i < tcount; i++) {
		...
		hexdump(result, template[i].dlen);
	}
...
static int alg_test_trng(const struct alg_test_desc *desc, const char *driver,
			  u32 type, u32 mask)
{
	struct crypto_rng *rng;
	int err;
	...
	err = test_trng(rng, desc->suite.trng.vecs, desc->suite.trng.count);
			     ...
			     const struct akcipher_testvec *vecs)
{
	...
	if(vecs->siggen_sigver_test) {
		...
		if (memchr_inv(outbuf_dec, 0, out_len - m_size) ||
			...
			hexdump(m, m_size);
			pr_err("******** 222 ********\n");
			hexdump(outbuf_dec + out_len - m_size, m_size);
...
static const struct alg_test_desc alg_test_descs[] = {
	{
		...
		.suite = {
			.cipher = {
				.enc = __VECS(osr_aes_cbc_enc_tv_template),
				.dec = __VECS(osr_aes_cbc_dec_tv_template)
			}
		}
	}, {
		.alg = "cbc(anubis)",
		...
		.alg = "cfb(3des-ede)-osr",
		.test = alg_test_skcipher,
		.fips_allowed = 1,
		.suite = {
			.cipher = {
				.enc = __VECS(osr_3des_ede_cfb_enc_tv_template),
				.dec = __VECS(osr_3des_ede_cfb_dec_tv_template)
			}
		}
	}, {
		.alg = "chacha20",
		...
		.suite = {
			.cipher = {
				.enc = __VECS(osr_sm4_ecb_enc_tv_template),
				.dec = __VECS(osr_sm4_ecb_dec_tv_template)
			}
		}
	},{
		.alg = "ecb(tea)",
		...
		.fips_allowed = 1,
		.suite = {
			.akcipher = __VECS(osr_rsa_nocrt_tv_template)
		}
	},{
		.alg = "salsa20",
		.test = alg_test_skcipher,
		...
		.test = alg_test_akcipher,
		.suite = {
			.akcipher = __VECS(osr_sm2_tv_template)
		}
	}, {
		.alg = "sm3",
		.test = alg_test_hash,
		...
		.test = alg_test_trng,
		.suite = {
			.trng = __VECS(trng_osr_tv_template)
		}
	}, {
		.alg = "vmac(aes)",
		.test = alg_test_hash,
...
int alg_test(const char *driver, const char *alg, u32 type, u32 mask)
{
	...
	pr_err("%s alg[%s] self test enable \n", __func__, alg);
