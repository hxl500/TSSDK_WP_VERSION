				...
				u32 inner_type)
{
	...
	//printk("func %s tag %d inner_tag %d inner_type %d p->des3 0x%x\r\n",__func__,tag,inner_tag,inner_type,p->des3);
}
...
static void dwmac4_set_vlan(struct dma_desc *p, u32 type)
{
	...
	//printk("func %s type 0x%x p->des2 0x%x \r\n",__func__,type,p->des2);
}
