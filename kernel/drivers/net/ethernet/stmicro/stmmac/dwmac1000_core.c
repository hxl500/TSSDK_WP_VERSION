				...
				struct net_device *dev)
{
	...
	stmmac_writel(value, ioaddr + GMAC_INT_MASK);
				 ...
				 int mcbitslog2)
{
	...
	switch (mcbitslog2) {
	case 6:
		stmmac_writel(mcfilterbits[0], ioaddr + GMAC_HASH_LOW);
		stmmac_writel(mcfilterbits[1], ioaddr + GMAC_HASH_HIGH);
		return;
		break;
	case 7:
		numhashregs = 4;
	...
	for (regs = 0; regs < numhashregs; regs++)
		stmmac_writel(mcfilterbits[regs],
...
static void dwmac1000_rgsmii(void __iomem *ioaddr, struct stmmac_extra_stats *x)
{
	...
	status = stmmac_readl(ioaddr + GMAC_RGSMIIIS);
	x->irq_rgmii_n++;
				...
				struct stmmac_extra_stats *x)
{
	...
	u32 intr_mask = stmmac_readl(ioaddr + GMAC_INT_MASK);
	int ret = 0;
...
{
	u32 value = stmmac_readl(ioaddr + GMAC_DEBUG);
