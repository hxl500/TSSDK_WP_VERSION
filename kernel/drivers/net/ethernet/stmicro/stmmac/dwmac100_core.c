...
static void dwmac100_dump_mac_regs(struct mac_device_info *hw, u32 *reg_space)
{
	...
	reg_space[MAC_CONTROL / 4] = stmmac_readl(ioaddr + MAC_CONTROL);
	reg_space[MAC_ADDR_HIGH / 4] = stmmac_readl(ioaddr + MAC_ADDR_HIGH);
	reg_space[MAC_ADDR_LOW / 4] = stmmac_readl(ioaddr + MAC_ADDR_LOW);
	reg_space[MAC_HASH_HIGH / 4] = stmmac_readl(ioaddr + MAC_HASH_HIGH);
	reg_space[MAC_HASH_LOW / 4] = stmmac_readl(ioaddr + MAC_HASH_LOW);
	reg_space[MAC_FLOW_CTRL / 4] = stmmac_readl(ioaddr + MAC_FLOW_CTRL);
	reg_space[MAC_VLAN1 / 4] = stmmac_readl(ioaddr + MAC_VLAN1);
	reg_space[MAC_VLAN2 / 4] = stmmac_readl(ioaddr + MAC_VLAN2);
}
