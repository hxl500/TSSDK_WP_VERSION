...
{
	u32 val = stmmac_readl(ioaddr + GMAC_AN_STATUS(reg));
...
{
	u32 value = stmmac_readl(ioaddr + GMAC_AN_CTRL(reg));

	if (restart)
		value |= GMAC_AN_CTRL_RAN;

	stmmac_writel(value, ioaddr + GMAC_AN_CTRL(reg));
}
...
{
	u32 value = stmmac_readl(ioaddr + GMAC_ANE_ADV(reg));
	...
	value = stmmac_readl(ioaddr + GMAC_ANE_LPA(reg));
