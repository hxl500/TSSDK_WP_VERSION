...
#if IS_ENABLED(CONFIG_MOTORCOMM_PHY_NOXTAL)
#define YTPHY8512_XTAL_INIT                (1)
#else
#define YTPHY8512_XTAL_INIT                (0)
#endif
...
#define YT8512_EXTREG_COMBO1            0x4000
...
#define YT8512_COMBO1_REMOTE_LOOPBACK   0x1000
...
#if (YTPHY8512_XTAL_INIT)
static int yt8512_xtal_init(struct phy_device *phydev)
{
	int ret = 0;
	int val = 0;
	...
	val = ytphy_read_ext(phydev, YT8512_EXTREG_COMBO1);
	if (val < 0)
		return val;

	/*rmii2 mode, if not do defaut rmii1 mode*/
	val &= ~YT8512_CONTROL1_RMII_EN;
	ret = ytphy_write_ext(phydev, YT8512_EXTREG_COMBO1, val);
	if (ret < 0)
		return ret;
...
static int yt8512_set_loopback(struct phy_device *phydev, bool enable)
{
	int ret;
	int val;

	pr_emerg("%s is %s \r\n",__func__, ((enable == true)? "enable" : "disable"));
	...
	if(enable) {
		...
		val |= YT8512_COMBO1_REMOTE_LOOPBACK;
		ret = ytphy_write_ext(phydev, YT8512_EXTREG_COMBO1, val);
		if (ret < 0)
			return ret;

		return phy_write(phydev, MII_BMCR, 0xa100);
	} else {
...
static struct phy_driver ytphy_drvs[] = {
	{
		...
		.suspend        = genphy_suspend,
		.resume         = genphy_resume,
		.set_loopback   = yt8512_set_loopback,
	}, {
