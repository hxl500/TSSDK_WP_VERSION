			     ...
			     unsigned int val, unsigned long flags, u8 width)
{
	...
	if (flags & CLK_DIVIDER_MUl_TWO)
		return ((val ? (val << 1) : 1));
	if (flags & CLK_DIVIDER_POWER_OF_TWO_PLUS_ONE)
...
#ifdef CONFIG_ARCH_TS
static struct clk_hw *_register_divider_with_sync(struct device *dev,
		const char *name,
		const char *parent_name, unsigned long flags,
 ...
 * @lock: shared register lock for this clock
 */
struct clk *clk_register_divider_with_sync(struct device *dev,
		const char *name,
		const char *parent_name, unsigned long flags,
		void __iomem *reg, u8 shift, u8 width, u8 we, u8 sync,
		u8 clk_divider_flags, spinlock_t *lock)
{
	...
	hw = _register_divider_with_sync(dev, name, parent_name, flags, reg,
		shift, width, we, sync, clk_divider_flags, NULL, lock);
	if (IS_ERR(hw))
		return ERR_CAST(hw);
	return hw->clk;
}
EXPORT_SYMBOL_GPL(clk_register_divider_with_sync);
#endif
