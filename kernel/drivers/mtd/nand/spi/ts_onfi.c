...
//extern int spinand_read_page(struct spinand_device *spinand,
...
#else
	.free = ts_onfi_8_ooblayout_free,
#endif
};

static int ts_onfi_4_ooblayout_ecc(struct mtd_info *mtd, int section,
				  struct mtd_oob_region *region)
{
...
static int ts_onfi_4_ooblayout_free(struct mtd_info *mtd, int section,
				   struct mtd_oob_region *region)
{
	struct spinand_device *spinand = mtd_to_spinand(mtd);
...
static const struct mtd_ooblayout_ops ts_onfi_4_ooblayout = {
	.ecc = ts_onfi_4_ooblayout_ecc,
	.free = ts_onfi_4_ooblayout_free,
};
...
static struct spinand_info ts_onfi_spinand_table[] = {
	...
	SPINAND_INFO("FM25LS01",
			SPINAND_ID(SPINAND_READID_METHOD_OPCODE_DUMMY, 0xA5),
		     NAND_MEMORG(1, 2048, 128, 64, 1024, 80, 1, 1, 1),
		     NAND_ECCREQ(8, 512),
	...
	SPINAND_INFO("onfi",
				SPINAND_ID(SPINAND_READID_METHOD_OPCODE_DUMMY, 0xff),
...
static int ts_onfi_spinand_detect(struct spinand_device *spinand)
{
	 ...
	 */
	if (id[0] != SPINAND_MFR_ONFI) {
		//return 0;
		pr_debug("Manufacturer id = 0x%x\n", *(uint32_t *)id);
	}
	...
	if (ret)
		return ret;
#else
	if (*(ts_onfi_spinand_table[onfi_index].devid.id) != id[1]) {
		id_val = id[1];
		ts_onfi_spinand_table[onfi_index].devid.id = &id_val;
	}

	ret = spinand_match_and_init(spinand, ts_onfi_spinand_table,
				     ARRAY_SIZE(ts_onfi_spinand_table), SPINAND_READID_METHOD_OPCODE_DUMMY);
	if (ret)
		return ret;
#endif
...
void spinand_fill_memorg(struct nand_onfi_params *page, struct spinand_info *onfi_info)
{
	struct nand_memory_organization memorg = NAND_MEMORG(page->bits_per_cell, page->byte_per_page,
		 page->spare_bytes_per_page, page->pages_per_block, page->blocks_per_lun, page->bb_per_lun,
	...
	struct nand_onfi_params *page, struct spinand_info *onfi_info)
{
	...
	info.offset = 0 << fls(nand->memorg.pagesize);
	...
	for (i = 0; i < 3; i++) {
		...
		if (ret) {
			pr_debug("read param error, ret=%d\n", ret);
			//spinand_reset_op(spinand);
			//continue;
		}
...
#else
		ret = spi_mem_dirmap_read(desc, i * sizeof(*page), sizeof(*page), page);
		if (ret < 0)
			return ret;
#endif
		...
		if (crc[i] != page->crc) {
			pr_warn("reading param crc=0x%x calc crc=0x%x\n", page->crc, crc[i]);
			ret = -1;
		} else {
			ret = 0;
			break;
	...
	spinand_fill_memorg(page, onfi_info);
...
static int ts_onfi_spinand_init(struct spinand_device *spinand)
{
	...
	if (ret)
		pr_debug("%s:%d Enable qual mode error! ret=%d\n", __func__, __LINE__, ret);

	ret = spinand_upd_cfg(spinand, CFG_OTP_ENABLE, CFG_OTP_ENABLE);
	if (ret)
		pr_debug("%s:%d update cfg error! ret=%d\n", __func__, __LINE__, ret);

	/* second, detect onfi. */
	ret = spinand_read_param_op(spinand, &param, &ts_onfi_spinand_table[onfi_index]);
	if (ret) {
		pr_debug("%s:%d read param error! ret=%d\n", __func__, __LINE__, ret);
	} else {
		for (i = 0; i <= sizeof(param); i++)
			pr_debug(" 0x%x ", ((uint8_t *)&param)[i]);
		...
		if (*(ts_onfi_spinand_table[onfi_index].devid.id) != id[1]) {
			id_val = id[1];
			pr_info("table->devid.id = 0x%x, id[1]=0x%x\n",
				*ts_onfi_spinand_table[onfi_index].devid.id, id[1]);
			ts_onfi_spinand_table[onfi_index].devid.id = &id_val;
		}
#endif
		/* reconfig. */
...
const struct spinand_manufacturer ts_onfi_spinand_manufacturer = {
	.id = SPINAND_MFR_ONFI,
	.name = "onfi",
	.chips = ts_onfi_spinand_table,
	.nchips = ARRAY_SIZE(ts_onfi_spinand_table),
