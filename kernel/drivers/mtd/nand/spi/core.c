					    ...
					    struct nand_page_io_req *req)
{
	...
	/* kowin spi nandflash special handle:ECC_EN must be always set to 1 */
	if (spinand->id.data[0] == kowin_spinand_manufacturer.id) {
		enable = true;
	}
				     ...
				     const struct nand_page_io_req *req)
{
	...
	if (req->datalen) {
		memcpy(spinand->databuf + req->dataoffs, req->databuf.out,
		       req->datalen);
		#if 0
		for (i = 0; i < req->datalen; i += 4) {
			printk("%x ", *(int *)(req->databuf.out + i));
			if (i % 0xc == 0) {
				printk("\n");
			}
		}
			...
			u8 *s)
{
	u8 status;
	int ret;
#if 1 //!defined(CONFIG_HIGH_RES_TIMERS) || 1
	unsigned long end =  jiffies + msecs_to_jiffies(SPINAND_WAITRDY_TIMEOUT_MS);
...
static bool spinand_isbad(struct nand_device *nand, const struct nand_pos *pos)
{
	...
	if (marker[0] != 0xff || marker[1] != 0xff) {
		pr_warn("badblock marker %d, %d in 0x%zu\n",
			marker[0], marker[1], pos->eraseblock * nanddev_eraseblock_size(nand));
		return true;
	}
...
static const struct spinand_manufacturer *spinand_manufacturers[] = {
	...
	&kowin_spinand_manufacturer,
	&heyangtek_spinand_manufacturer,
	&foresee_spinand_manufacturer,
	&ts_onfi_spinand_manufacturer,
};
				      ...
				      enum spinand_readid_method rdid_method)
{
	...
	for (i = 0; i < ARRAY_SIZE(spinand_manufacturers); i++) {
		if (spinand_manufacturers[i]->ops->detect != NULL) {
		...
		manufacturer = spinand_manufacturers[i];
			   ...
			   enum spinand_readid_method rdid_method)
{
	...
	for (i = 0; i < table_size; i++) {
		...
		if (memcmp(id + 1, info->devid.id, info->devid.len)) {
			pr_debug("id+1 = 0x%x, table->devid.id=0x%x\n", *(id + 1), *info->devid.id);
			continue;
		}
