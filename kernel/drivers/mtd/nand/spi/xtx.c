...
static int xtx_spinand_detect(struct spinand_device *spinand)
{
	...
	int ret;
	u8 index = ARRAY_SIZE(xtx_spinand_table) - 1;
	...
	if (ret)
		return ret;
#else
	if (*(xtx_spinand_table[index].devid.id) != id[1]) {
		id_val = id[1];
		//xtx_spinand_table[index].devid.id = &id_val;
	}

	ret = spinand_match_and_init(spinand, xtx_spinand_table,
				     ARRAY_SIZE(xtx_spinand_table), SPINAND_READID_METHOD_OPCODE_ADDR);
	if (ret)
		return ret;
#endif
