...
static void dw_i2c_set_fifo_size(struct dw_i2c_dev *dev)
{
	...
	if ((dev->rx_fifo_depth == 0) || (dev->tx_fifo_depth == 0)) {
		/*
...
static int dw_i2c_plat_probe(struct platform_device *pdev)
{
	...
	device_property_read_u32(&pdev->dev, "tx-depth",
			&dev->tx_fifo_depth);
	device_property_read_u32(&pdev->dev, "rx-depth",
			&dev->rx_fifo_depth);
	...
	dev->clk = devm_clk_get(&pdev->dev, "icclk");
	if (IS_ERR(dev->clk)) {
	...
	if (IS_ERR(dev->pclk))
		dev_err(&pdev->dev, "clk_get pclk maybe not config\n");
	else
