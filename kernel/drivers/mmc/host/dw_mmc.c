...
#include "dw_mmc.h"
#include "dw_mmc_tsingmicro.h"

struct dw_mci_ts_priv_data {
	struct clk		*drv_clk;
...
static void dw_mci_setup_bus(struct dw_mci_slot *slot, bool force_clkinit)
{
	...
	if (!clock) {
		...
		mci_writel(host, CLKDIV, (div == 0) ? 1 : div);
...
#define TS_SDHC_EMMC_RUN_HS200
int dw_mci_ts_execute_tuning(struct dw_mci_slot *slot, u32 opcode)
{
	...
	for (i = 0; i < priv->num_phases; ) {
		clk_set_phase(priv->sample_clk,     //TODO: need to make sure which clk should be used driver
...
EXPORT_SYMBOL(dw_mci_ts_execute_tuning);

#ifdef CONFIG_OF
static int dw_mci_ts_parse_dt(struct dw_mci *host)
{
	struct dw_mci_ts_priv_data *priv;
	...
	if (of_property_read_u32(np, "ts,desired-num-phases",
				&priv->num_phases))
	priv->num_phases = 360;

	if (of_property_read_u32(np, "ts,default-sample-phase",
					&priv->default_sample_phase))
		priv->default_sample_phase = 0;
	priv->sample_clk = devm_clk_get(host->dev, "smpl_clk");
...
static struct dw_mci_board *dw_mci_parse_dt(struct dw_mci *host)
{
	...
	pdata->rstc = devm_reset_control_get_optional_exclusive(dev, "sdhci");
	if (IS_ERR(pdata->rstc)) {
		if (PTR_ERR(pdata->rstc) == -EPROBE_DEFER)
			return ERR_PTR(-EPROBE_DEFER);
	}

	dw_mci_ts_parse_dt(host);
...
int dw_mci_probe(struct dw_mci *host)
{
	...
	host->siu_clk = devm_clk_get(host->dev, "siu");
	if (IS_ERR(host->siu_clk)) {
		dev_dbg(host->dev, "siu clock not available\n");
	} else {
		ret = clk_prepare_enable(host->siu_clk);
		if (ret) {
			dev_err(host->dev, "failed to enable siu clock\n");
			return ret;
		}
	}

	host->diu_clk = devm_clk_get(host->dev, "diu");
	if (IS_ERR(host->diu_clk)) {
		dev_dbg(host->dev, "diu clock not available\n");
	} else {
		ret = clk_prepare_enable(host->diu_clk);
		if (ret) {
			dev_err(host->dev, "failed to enable diu clock\n");
			return ret;
		}
	}
#endif
	...
	if (IS_ERR(host->hiu_clk)) {
		dev_err(host->dev, "hiu clock not available\n");
	} else {
		ret = clk_prepare_enable(host->hiu_clk);
		if (ret) {
			dev_err(host->dev, "failed to enable hiu clock\n");
			return ret;
		}
	}
...
err_clk_biu:
#endif
	clk_disable_unprepare(host->hiu_clk);
