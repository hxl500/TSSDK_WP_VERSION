...
static struct drm_encoder *virt_hdmi_connector_best_encoder(struct drm_connector *connector)
{
	return &encoder;
}
				 ...
				 void *data)
{
	...
	ret = drm_encoder_init(drm, &encoder, &virt_hdmi_encoder_funcs, DRM_MODE_ENCODER_VIRTUAL, NULL);
	if (ret) {
		dev_err(dev, "Failed to initialize encoder with drm\n");
		return ret;
	}
	ret = drm_connector_init(drm, &connector, &virt_hdmi_connector_funcs, DRM_MODE_CONNECTOR_VIRTUAL);
	if (ret) {
