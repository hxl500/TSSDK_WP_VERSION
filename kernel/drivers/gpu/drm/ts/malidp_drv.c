...
static void malidp_atomic_commit_hw_done(struct drm_atomic_state *state)
{
	...
	if (malidp->crtc.enabled) {
		...
		if (malidp_set_and_wait_config_valid(drm))
			DRM_DEBUG_DRIVER("timed out waiting for updated cfg\n");
	}
				       ...
				       const struct of_device_id *dev_id)
{
	...
	if (is_dp500 != dt_is_dp500) {
		...
		if (!strnstr(dev_id->compatible, buf,
			...
			DRM_ERROR("Device-tree expects %s, but hw is DP%03X.\n",
...
static int malidp_bind(struct device *dev)
{
	...
	{
		DRM_ERROR("Unable to get dp0 pclk clk: %d\n", ret);
		goto err_pclk;
	}
	ret = clk_prepare_enable(hwdev->pclk);
	if (ret) {
		DRM_ERROR("Cannot enable dp0 pclk clock: %d\n", ret);
		goto err_pclk;
	}
	...
	{
		DRM_ERROR("Unable to get dp0 aclk clk: %d\n", ret);
		goto err_aclk;
	}
	ret = clk_prepare_enable(hwdev->aclk);
	if (ret) {
		DRM_ERROR("Cannot enable dp0 aclk clock: %d\n", ret);
		goto err_aclk;
	}
	...
	{
		DRM_ERROR("Unable to get dp0 mclk clk: %d\n", ret);
		goto err_mclk;
	}
	ret = clk_prepare_enable(hwdev->mclk);
	if (ret) {
		DRM_ERROR("Cannot enable dp0 mclk clock: %d\n", ret);
		goto err_mclk;
	}
	...
	{
		DRM_ERROR("Unable to get dp0 pxlclk clk: %d\n", ret);
		goto err_pxlclk;
	}
	ret = clk_prepare_enable(hwdev->pxlclk);
	if (ret) {
		DRM_ERROR("Cannot enable dp0 pxlclk clock: %d\n", ret);
		goto err_pxlclk;
	}
	...
	drm->mode_config.delayed_event = false;
