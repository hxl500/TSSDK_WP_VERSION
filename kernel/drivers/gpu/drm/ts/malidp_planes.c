...
#define MALIDP_LAYER_YUV2RGB		0x084

/* from IP test case */
const static u32 yuv2rgb_coeff[] = {
	0x4a8, 0x000, 0x664, 0x4a8, 0x1e70, 0x1cc0,
	0x4a8, 0x814, 0x0, 0x40, 0x200, 0x200
};
...
int malidp_de_planes_init(struct drm_device *drm)
{
	...
	for (i = 0; i < map->n_layers; i++) {
		...
		if (id == DE_SMART) {
			...
			/*
			 * Skip the features which the SMART layer doesn't
			 * have.
			 */
			continue;
		}
		...
		/* YUV2RGB coff */
		if ((id == DE_VIDEO1) || (id == DE_VIDEO2)) {
			for (j = 0; j < ARRAY_SIZE(yuv2rgb_coeff); j++) {
				reg = plane->layer->base + MALIDP_LAYER_YUV2RGB
					+ 4 * j;
				malidp_hw_write(malidp->dev, yuv2rgb_coeff[j],
					reg);
			}
		}
