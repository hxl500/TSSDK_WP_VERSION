...
#define MALIDP_COMMON_FORMATS \
	...
	{ DRM_FORMAT_BGRA1010102, DE_VIDEO1 | DE_GRAPHICS1 | DE_VIDEO2, \
		MALIDP_ID(0, 3) }, \
	{ DRM_FORMAT_ARGB8888, \
		DE_VIDEO1 | DE_GRAPHICS1 | DE_VIDEO2 | DE_SMART, \
		MALIDP_ID(1, 0) }, \
	{ DRM_FORMAT_ABGR8888, \
		DE_VIDEO1 | DE_GRAPHICS1 | DE_VIDEO2 | DE_SMART, \
		MALIDP_ID(1, 1) }, \
	...
	{ DRM_FORMAT_ABGR1555, DE_VIDEO1 | DE_GRAPHICS1 | DE_VIDEO2, \
		MALIDP_ID(4, 1) }, \
	{ DRM_FORMAT_RGB565, DE_VIDEO1 | DE_GRAPHICS1 | DE_VIDEO2, \
		MALIDP_ID(4, 2) }, \
	{ DRM_FORMAT_BGR565, DE_VIDEO1 | DE_GRAPHICS1 | DE_VIDEO2, \
		MALIDP_ID(4, 3) }, \
...
static int malidp550_rotmem_required(struct malidp_hw_device *hwdev,
	u16 w, u16 h, u32 fmt)
{
	u32 bytes_per_col;
...
static int malidp550_se_set_scaling_coeffs(
	struct malidp_hw_device *hwdev, struct malidp_se_config *se_config,
	struct malidp_se_config *old_config)
{
				   ...
				   struct videomode *vm)
{
	...
	ret = clk_get_rate(hwdev->mclk);
	if (ret < mclk) {
		DRM_WARN("mclk requirement of %lu(%lu) kHz can't be met.\n",
				mclk / 1000, ret/1000);
	}
	return ret;
}
		...
		.map = {
			...
			*/
				.irq_mask = MALIDP_DE_IRQ_UNDERRUN |
					    ...
					    MALIDP550_DE_IRQ_AXI_ERR_MMU |
					    MALIDP550_DE_IRQ_VSYNC,
				.vsync_irq = MALIDP550_DE_IRQ_VSYNC,
				.irq_error = MALIDP_DE_IRQ_UNDERRUN |
					    MALIDP650_DE_IRQ_DRIFT |
					    MALIDP550_DE_IRQ_SATURATION |
					    MALIDP550_DE_IRQ_AXI_ERR |
					    MALIDP550_DE_IRQ_AXI_ERR_MMU,
...
static irqreturn_t malidp_de_irq(int irq, void *arg)
{
	...
	dc_status = malidp_hw_read(hwdev, hwdev->map.dc_base
		+ MALIDP_REG_STATUS);
	pr_debug("malidp dc status %x,", dc_status);
	...
	pr_debug("malidp de status %x,", status);
	if (status & de->irq_error) {
		pr_info("malidp de status error %x,", status);
		malidp_hw_clear_irq(hwdev, MALIDP_DE_BLOCK, status);	// TODO
		return ret;
	}
	if (!(status & de->irq_mask)) {
		malidp_hw_clear_irq(hwdev, MALIDP_DE_BLOCK, status);	// TODO
		pr_debug("malidp de no intrest int %x,", status);
		return ret;
	}
