...
static struct usb_interface_descriptor uvc_control_intf = {
	...
	.bAlternateSetting	= 0,
#if USE_UVC_XU
	.bNumEndpoints		= 1,   /////VC 0
#else
	.bNumEndpoints		= 0,   /////VC 0
#endif
...
#else
	.bLength		= 0,//vc_USB_DT_ENDPOINT_SIZE,
#endif
...
#else
	.bLength		= 0,//UVC_DT_CONTROL_ENDPOINT_SIZE,  //VC
#endif
...
uvc_function_ep0_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	//dev_err(&uvc->func.config->cdev->gadget->dev, "%s req->actual =%d,uvc->event_setup_out =%d\n",__func__,req->actual,uvc->event_setup_out);
...
uvc_function_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
	...
	dev_err(&f->config->cdev->gadget->dev, "%s setup request %02x %02x value %04x index %04x %04x\n",__func__,ctrl->bRequestType, ctrl->bRequest, le16_to_cpu(ctrl->wValue),
		le16_to_cpu(ctrl->wIndex), le16_to_cpu(ctrl->wLength));
    ...
    if(ctrl->bRequestType == 0x21 && ctrl->bRequest == 0x1 && 0x1a == le16_to_cpu(ctrl->wLength)){
		//dev_err(&f->config->cdev->gadget->dev, "%s bRequestType =0x21\n",__func__);
        //return 0;
	}
	memset(&v4l2_event, 0, sizeof(v4l2_event));
...
uvc_function_get_alt(struct usb_function *f, unsigned interface)
{
	...
	//dev_err(&f->config->cdev->gadget->dev, "%s interface = (%u)\n",__func__, interface);
...
uvc_function_set_alt(struct usb_function *f, unsigned interface, unsigned alt)
{
	...
	if(alt){
		dev_err(&f->config->cdev->gadget->dev, "%s interface = (%u %d)\n",__func__, interface,alt);
	}
	...
	*/
	...
	case 1:
		...
		INFO(cdev, " %s (disable) UVC ep\n",__func__);
		uvc->video.ep->enabled = 1;
		usb_ep_disable(uvc->video.ep);
		uvc->video.ep->enabled = 0;
...
#if USE_UVC_XU
	UVC_COPY_DESCRIPTOR(mem, dst, &uvc_control_ep);  ///// vc_if_0
#endif
	if (speed == USB_SPEED_SUPER)
		UVC_COPY_DESCRIPTOR(mem, dst, &uvc_ss_control_comp);
#if USE_UVC_XU
	UVC_COPY_DESCRIPTOR(mem, dst, &uvc_control_cs_ep);  ///// vc_if_0
#endif
...
uvc_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	INFO(cdev, "######max_packet_mult =%d,max_packet_size =%d#######\n",max_packet_mult,max_packet_size);
	INFO(cdev, "######opts->streaming_interval =%d#######\n",opts->streaming_interval);
	...
	INFO(cdev, "######uvc->video.ep->address =0x%x#######\n",uvc->video.ep->address);
...
static struct usb_function_instance *uvc_alloc_inst(void)
{
	...
	ed->bNumControls		= 5;	/* totally 15 controls - reserved for customer */
	ed->bNrInPins			= 1;	/* 1 Input pin (PU) */
	ed->baSourceID[0]		= 2;
	ed->bControlSize		= 2;	/* 2 bmControls */
	ed->bmControls[0]		= 0x1f;	/* bitmap for supported control */
	ed->bmControls[1]		= 0;
