...
extern unsigned int p_uac_send_control_flag;
extern unsigned int p_uac_recv_control_flag;

#if iad_inter
static struct usb_interface_assoc_descriptor uac1_iad_descriptor = {
	.bLength =		sizeof uac1_iad_descriptor,
	.bDescriptorType =	USB_DT_INTERFACE_ASSOCIATION,
 
	/* .bFirstInterface =	DYNAMIC, */
	.bInterfaceCount = 	3,	// control + data
	.bFunctionClass =	USB_CLASS_AUDIO,
	.bFunctionSubClass =	USB_CLASS_AUDIO,
	.bFunctionProtocol =	0,
	/* .iFunction =		DYNAMIC */
};
...
#if iad_inter
	(struct usb_descriptor_header *) &uac1_iad_descriptor,
#endif
...
f_audio_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
	...
	//dev_err(dev, "%s:bRequestType 0x%x bRequest 0x%x, w_value 0x%04x, len %d, wIndex 0x%x#####\n", 
	                                         //__func__,ctrl->bRequestType,ctrl->bRequest, le16_to_cpu(ctrl->wValue), le16_to_cpu(ctrl->wLength), le16_to_cpu(ctrl->wIndex));
	...
	if(ctrl->bRequestType == 0x22 && ctrl->bRequest == 0x01 && (le16_to_cpu(ctrl->wIndex)== 0x82 || le16_to_cpu(ctrl->wIndex)== 0x83) ){
			p_uac_send_control_flag = 1;
			dev_err(dev,"%s:p_uac_send_control_flag = %d, #####\n", __func__,p_uac_send_control_flag);
	}

	if(ctrl->bRequestType == 0x22 && ctrl->bRequest == 0x01 && le16_to_cpu(ctrl->wIndex)== 0x1 ){
			p_uac_recv_control_flag = 1;
			//dev_err(dev,"%s:p_uac_recv_control_flag =  %d, #####\n", __func__,p_uac_recv_control_flag);
	}
...
static int f_audio_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	int ret = 0;
	if(alt){
	dev_err(dev, "f_audio_set_alt(interface %u alt %u)\n", intf,alt);
	}
	...
	if (intf == uac1->as_out_intf) {
		...
		if (alt){
			...
			p_uac_recv_control_flag = 0;
			u_audio_stop_capture(&uac1->g_audio);
			//dev_err( dev,"%s: capture p_uac_recv_control_flag == %d, #####\n", __func__,p_uac_recv_control_flag);
		}
		...
		uac1->as_in_alt = alt;
			...
			if (alt){
				...
				p_uac_send_control_flag = 0;
				u_audio_stop_playback(&uac1->g_audio);
				//dev_err(dev, "%s:play p_uac_send_control_flag == %d, #####\n", __func__,p_uac_send_control_flag);
			}
	} else {
		dev_err(dev, "%s:%d Error!\n", __func__, __LINE__);
		return -EINVAL;
...
static int f_audio_bind(struct usb_configuration *c, struct usb_function *f)
{
...
#if iad_inter
	uac1_iad_descriptor.iFunction = us[STR_AC_IF].id;
#endif
	...
	if (status < 0)
		goto fail;
#if iad_inter
	uac1_iad_descriptor.bFirstInterface = status;
#endif
	...
	if (!ep)
		goto fail;
	audio->out_ep = ep;
	//dev_err(dev, "%s as_out_ep_desc.bInterval = %d \n",__func__,as_out_ep_desc.bInterval);
	...
	if (!ep)
		goto fail;
	audio->in_ep = ep;
	//dev_err(dev, "%s as_in_ep_desc.bInterval = %d \n",__func__,as_in_ep_desc.bInterval);	
