				 ...
				 unsigned int en)
{
	...
	//dev_dbg(hsotg->dev, "%s: ep =%d dir =%s daint_0x818=0x%08x ,daintmask_0x81c=0x%08x\n",
	//	__func__,ep,dir_in ? "in" : "out",dwc2_readl(hsotg->regs + DAINT), daint);
	//dev_dbg(hsotg->dev, "%s: ep =%d dir =%s DIEPMSK_0x810=0x%08x ,DOEPMSK_0x814=0x%08x\n",
	//__func__,ep,dir_in ? "in" : "out", dwc2_readl(hsotg->regs + DIEPMSK),dwc2_readl(hsotg->regs + DOEPMSK));
...
int dwc2_hsotg_tx_fifo_count(struct dwc2_hsotg *hsotg)
{
	...
	//dev_dbg(hsotg->dev, "%s line:%d en_multiple_tx_fifo =%d", __FUNCTION__,__LINE__ ,hsotg->hw_params.en_multiple_tx_fifo);
	...
	//dev_dbg(hsotg->dev, "%s line:%d num_dev_perio_in_ep =%d", __FUNCTION__,__LINE__ ,hsotg->hw_params.num_dev_perio_in_ep);

		return hsotg->hw_params.num_dev_perio_in_ep + 1;
	}
}
...
int dwc2_hsotg_tx_fifo_total_depth(struct dwc2_hsotg *hsotg)
{
	...
	//dev_dbg(hsotg->dev, "%s line:%d np_tx_fifo_size =%d", __FUNCTION__,__LINE__ ,np_tx_fifo_size);

	/* Get Endpoint Info Control block size in DWORDs. */
	ep_info_size = dwc2_hsotg_ep_info_size(hsotg);
	dev_dbg(hsotg->dev, "%s ep_info_size =%d,total_fifo_size =%d", __FUNCTION__,ep_info_size,hsotg->hw_params.total_fifo_size);

	tx_addr_max = hsotg->hw_params.total_fifo_size - ep_info_size;

	//dev_dbg(hsotg->dev, "%s line:%d ep_info_size =%d,tx_addr_max =%d", __FUNCTION__,__LINE__ ,ep_info_size,tx_addr_max);
...
int dwc2_hsotg_tx_fifo_average_depth(struct dwc2_hsotg *hsotg)
{
	...
	tx_fifo_count = dwc2_hsotg_tx_fifo_count(hsotg);  //per_in_ep
...
static void dwc2_hsotg_init_fifo(struct dwc2_hsotg *hsotg)
{
	...
	for(fifo_num = 1;fifo_num <=5;fifo_num++)
	{
	 //dev_dbg(hsotg->dev, "%s line:%d txfsz[fifo_num =%d] =%d", __FUNCTION__,__LINE__,fifo_num,txfsz[fifo_num]);
	}
	fifo_num = dwc2_hsotg_tx_fifo_count(hsotg);
	...
	dev_dbg(hsotg->dev, "%s line:%d addr_size = %d ", __FUNCTION__,__LINE__,addr);
	dev_dbg(hsotg->dev, "hsotg->params.g_tx_fifo_size[1]= %d ", hsotg->params.g_tx_fifo_size[1]);
	...
	for (ep = 1; ep <= fifo_num; ep++) {
		if (!txfsz[ep])
			continue;
		val = addr;
		val |= txfsz[ep] << FIFOSIZE_DEPTH_SHIFT;
		dev_err(hsotg->dev, "%s DPTXFSIZN(%d) ,val =0x%08x", __FUNCTION__ ,ep,val);
		WARN_ONCE(addr + txfsz[ep] > hsotg->fifo_mem,
			  "insufficient fifo memory");
		addr += txfsz[ep];
		dev_err(hsotg->dev, "%s DPTXFSIZN(%d) ,val =0x%08x,addr=0x%08x", __FUNCTION__ ,ep,val,addr);

		dwc2_writel(val, hsotg->regs + DPTXFSIZN(ep));
		val = dwc2_readl(hsotg->regs + DPTXFSIZN(ep));
		dev_err(hsotg->dev, "%s DPTXFSIZN(%d) ,val =0x%08x", __FUNCTION__ ,ep,val);
	}
	dev_err(hsotg->dev, "%s all ep total_fifo_size =0x%08x", __FUNCTION__ ,addr);
				...
				bool continuing)
{
	...
	//dev_dbg(hsotg->dev, "%s:  ep %d, dir %s\n",__func__,  index,hs_ep->dir_in ? "in" : "out");
	...
	if(dir_in && index && !hs_ep->periodic){
	...
	//dev_dbg(hsotg->dev, "%s: epsize:: 0x%08x => reg:0x%08x\n",__func__,  epsize, epsize_reg);	
	/*
	...
	if(index){
		//dev_dbg(hsotg->dev, "%s:  epsize :%d@%d/%d, value:0x%08x => reg:0x%08x\n",__func__, packets, length, ureq->length, epsize, epsize_reg);
	}
	...
	if (using_desc_dma(hsotg)) {
		...
		if(index){
		//dev_dbg(hsotg->dev, "%s: epsize_reg=0x%08x\n", __func__, epsize);
			}
		...
		if (using_dma(hsotg) && !continuing && (length != 0)) {
			...
			//dev_dbg(hsotg->dev, " %s:dma_addr111:  ureq->dma = 0x%08x\n",__func__, ureq->dma);


			dwc2_writel(ureq->dma, hsotg->regs + dma_reg);

			//dev_dbg(hsotg->dev, " %s:&dma_addr: %p => 0x%08x\n",__func__, &ureq->dma, dma_reg);
		}
	}


	//if ( !hsotg->dedicated_fifos && !hs_ep->periodic && using_dma(hsotg) && hs_ep->dir_in){
	//	    ctrl |=   DXEPCTL_NEXTEP(index) ;
	//}
	...
	if (!(index == 0 && hsotg->ep0_state == DWC2_EP0_SETUP)){
		...
		/////new_add
	if(!hs_ep->periodic && index ){	
		ctrl &=   (~DXEPCTL_SNAK);
		if(index && dir_in){
		//ctrl |=  DXEPCTL_SETD0PID ;	/* clear NAK set by core */
		 //ctrl &=   (~(DXEPCTL_NAKSTS | DXEPCTL_DPID));
		  ctrl |=   DXEPCTL_NEXTEP(index);
		}
		if(index){
	...
	//dev_dbg(hsotg->dev, "%s:  (ctrl=0x%08x)\n",__func__, ctrl);


	dwc2_writel(ctrl, hsotg->regs + epctrl_reg);
	//dev_dbg(hsotg->dev, "%s:  ep %d, dir %s,(DXEPCTL=0x%08x)\n",__func__,  index,hs_ep->dir_in ? "in" : "out",dwc2_readl(hsotg->regs + epctrl_reg));
	 ...
	 */  //new_add
	if (dir_in){
			//dwc2_writel(DIEPMSK_INTKNTXFEMPMSK, hsotg->regs + DIEPINT(index));
     }
	...
	if (!(dwc2_readl(hsotg->regs + epctrl_reg) & DXEPCTL_EPENA)){
		//dev_dbg(hsotg->dev,"ep%d: failed to become enabled (DXEPCTL=0x%08x)?\n",
			       ...
			       gfp_t gfp_flags)
{
	...
	//dev_dbg(hs->dev, " %s %s: req %p: %d@buffer= 0x%p, noi=%d, zero=%d, snok=%d\n",
		//__func__,ep->name, req, req->length, req->buf, req->no_interrupt,
...
static void dwc2_hsotg_enqueue_setup(struct dwc2_hsotg *hsotg)
{
	...
	//dev_dbg(hsotg->dev, "%s: queueing setup request %s\n", __func__,hsotg->eps_out[0]->ep.name);
...
static void dwc2_hsotg_rx_data(struct dwc2_hsotg *hsotg, int ep_idx, int size)
{
	...
	//void __iomem *fifo = hsotg->regs + EPFIFO(0);
	int to_read;
	int max_req;
	int read_ptr;
...
static void dwc2_hsotg_handle_rx(struct dwc2_hsotg *hsotg)
{
	...
	//dev_dbg(hsotg->dev, "%s: GRXSTSP=0x%08x (size = %d@ep = %d)\n",
				       ...
				       bool is_usb_reset)
{
	...
	if (hsotg->params.phy_type == DWC2_PHY_TYPE_PARAM_FS &&
		...
		dev_dbg(hsotg->dev, "%s ####usbcfg=0x%08x\n",__func__,usbcfg);
	} else {
		/* set the PLL on, remove the HNP/SRP and set the PHY */
		val = (hsotg->phyif == GUSBCFG_PHYIF8) ? 9 : 5;
		//usbcfg |= hsotg->phyif | GUSBCFG_TOUTCAL(7) |(val << GUSBCFG_USBTRDTIM_SHIFT);
		usbcfg |= hsotg->phyif |(val << GUSBCFG_USBTRDTIM_SHIFT);
			dev_dbg(hsotg->dev, "%s ====usbcfg=0x%08x\n",__func__,usbcfg);
	}
...
#if 0
	dwc2_writel(((hsotg->dedicated_fifos && !using_dma(hsotg)) ?
		DIEPMSK_TXFIFOEMPTY : 0) | DIEPMSK_INTKNTXFEMPMSK |
		DIEPMSK_EPDISBLDMSK | DIEPMSK_XFERCOMPLMSK |
		DIEPMSK_TIMEOUTMSK | DIEPMSK_AHBERRMSK,
		hsotg->regs + DIEPMSK);
#endif
	...
	if (using_dma(hsotg)){
			//dwc2_writel(dwc2_readl(hsotg->regs + DCTL) | DCTL_IFRMNUN , hsotg->regs + DCTL);  //new_add
	}
...
irq_retry:
	...
	//dev_dbg(hsotg->dev, "%s: int:0x%08x int & mask:0x%08x (0x%08x) retry %d\n",
	...
	if (gintsts & (GINTSTS_OEPINT | GINTSTS_IEPINT)) {
		...
		//dev_dbg(hsotg->dev, "%s: daint=0x%08x,daintmsk = 0x%08x\n", __func__, daint,daintmsk);
	...
	if (gintsts & GINTSTS_RXFLVL) {
		 ...
		 * set.
		 */
		dev_dbg(hsotg->dev, " %s GINTSTS_RXFLVL\n",__func__);

		dwc2_hsotg_handle_rx(hsotg);
	}

	if (gintsts & GINTSTS_ERLYSUSP) {
		dev_dbg(hsotg->dev, "%s GINTSTS_ErlySusp\n",__func__);
	...
	if (gintsts & GINTSTS_GOUTNAKEFF) {
       ...
       ///////
		dev_dbg(hsotg->dev, "%s GOUTNakEff triggered\n",__func__);
		for (idx = 1; idx <= hsotg->num_of_eps; idx++) {
			hs_ep = hsotg->eps_out[idx];
			epctrl = dwc2_readl(hsotg->regs + DOEPCTL(idx));
	...
	if (gintsts & GINTSTS_GINNAKEFF) {
		dev_info(hsotg->dev, "%s GINNakEff triggered\n",__func__);
				...
				const struct usb_endpoint_descriptor *desc)
{
	...
	u32 mask;
	unsigned int dir_in;
	unsigned int i, val, size,rx_value,np_value;
	int ret = 0;
	...
	//dev_dbg(hsotg->dev,"%s: mps %d  mc %d\r\n",__func__, mps,mc);
	 ...
	 */
	if ((dir_in && hsotg->dedicated_fifos) ||(dir_in && hs_ep->isochronous)) {
		...
		for (i = 1; i <= fifo_count; ++i) {
			...
			rx_value = dwc2_readl(hsotg->regs + GRXFSIZ); //320
		    np_value = dwc2_readl(hsotg->regs + GNPTXFSIZ);//144
		...
		//fifo_size = (1004 - rx_value - np_value) *4;

		hsotg->fifo_map |= 1 << fifo_index;
		epctrl |= DXEPCTL_TXFNUM(fifo_index);
		hs_ep->fifo_index = fifo_index;
		hs_ep->fifo_size = fifo_size;
	    dev_dbg(hsotg->dev, "%s: fifo_count=%d,fifo_map =%d,fifo_size =%d\n",__func__,fifo_count,hsotg->fifo_map,hs_ep->fifo_size);			
	}
	//dev_dbg(hsotg->dev, "%s: hs_ep->fifo_size  =%d,hs_ep->fifo_index=%d,hs_ep->fifo_map=%d\n",__func__, hs_ep->fifo_size ,hs_ep->fifo_index,hsotg->fifo_map);
   ...
   //dev_dbg(hsotg->dev, "%s: ep =%d,dir %s\n",__func__, index,dir_in ? "in":"out");
	...
	if (hsotg->params.speed  == DWC2_SPEED_PARAM_FULL &&
		...
		if ((gsnpsid >= DWC2_CORE_REV_2_71a &&
...
static int dwc2_hsotg_ep_disable(struct usb_ep *ep)
{
	...
	ctrl = dwc2_readl(hsotg->regs + epctrl_reg);
	dev_dbg(hsotg->dev, "%s:dir %s before DxEPCTL=0x%08x\n", __func__, dir_in ? "in":"out",ctrl);
	...
	dev_dbg(hsotg->dev, "%s:dir %s after DxEPCTL=0x%08x\n", __func__, dir_in ? "in":"out",ctrl);
...
static void dwc2_hsotg_init(struct dwc2_hsotg *hsotg)
{
	...
	trdtim = (hsotg->phyif == GUSBCFG_PHYIF8) ? 9 : 5;
 	usbcfg |= hsotg->phyif | GUSBCFG_TOUTCAL(7) | (trdtim << GUSBCFG_USBTRDTIM_SHIFT);
	//usbcfg |= hsotg->phyif | (trdtim << GUSBCFG_USBTRDTIM_SHIFT);
	dwc2_writel(usbcfg, hsotg->regs + GUSBCFG);
	dev_dbg(hsotg->dev, "usbcfg=0x%08x\n",usbcfg);
...
static int dwc2_hsotg_pullup(struct usb_gadget *gadget, int is_on)
{
	...
	dev_dbg(hsotg->dev, "%s: is_on: %d op_state: %d\n", __func__, is_on,
		hsotg->op_state);
	dev_dbg(hsotg->dev, "%s %d", __func__,__LINE__);
...
static void dwc2_hsotg_dump(struct dwc2_hsotg *hsotg)
{
	...
	dev_info(dev, "REG_0X800_DCFG=0x%08x, REG_0X804_DCTL=0x%08x, REG_0X810_DIEPMSK=%08x\n",
	...
	dev_info(dev, "REG_0X828_DVBUSDIS=0x%08x, REG_0X82c_DVBUSPULSE=%08x\n",
