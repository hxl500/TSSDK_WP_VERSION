...
static void ts_tx5112_pin_to_offset(unsigned int pin,
...
static void ts_tx5112_gpio_func_sel(struct pinctrl_dev *pctldev, unsigned int pin,
	unsigned int func_selector)
{
	...
	//pr_err("debug_info:pin%d, wr 0x%p, val 0x%x\n", pin, pctrl->reg_base , val);
}
							  ...
							  unsigned int function, unsigned int group)
{
		...
		for (i = 0; i < pgrp->npins; i++)
			ts_tx5112_gpio_func_sel(pctldev, pgrp->pins[i], pgrp->mux_val);
					...
					unsigned int offset)
{
	...
	ts_tx5112_gpio_func_sel(pctldev, offset, TS_PINMUX_GPIO_MUX_VAL);
				   ...
				   unsigned int pin, unsigned long *config)
{
	...
	ts_tx5112_pin_to_offset(pin, &pad_offset, &reg_offset);
...
static const struct of_device_id ts_pinctrl_of_match[] = {
	{.compatible = "ts,ts-pinctrl-tx5112"},
	{}
};
