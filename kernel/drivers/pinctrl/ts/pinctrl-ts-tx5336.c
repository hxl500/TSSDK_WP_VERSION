...
#define TS_PINMUX_GPIO_MUX_VAL 5
...
/*gpio port control*/
#define SCD_GPIO_PCTL_REG		0x2c
// /*gpio rate control select*/
// #define SCD_GPIO_SLR_REG		0x18
// /*gpio digital enabel*/
// #define SCD_GPIO_DEN_REG		0x1c
// /*gpio analog mode select*/
// #define SCD_GPIO_AMSEL_REG		0x28
#define SCD_GPIO_SL_REG         0x44
#define SCD_GPIO_MSC_REG        0x48
#define SCD_GPIO_STB_REG        0x4c
...
static void ts_tx5336_gpio_func_sel(struct pinctrl_dev *pctldev, unsigned int pin,
	unsigned int func_selector)
{
	...
	pr_debug("debug_info:pin%d, wr 0x%p, val 0x%x\n", pin, pctrl->reg_base , val);
}
							  ...
							  unsigned int function, unsigned int group)
{
		...
		for (i = 0; i < pgrp->npins; i++)
			ts_tx5336_gpio_func_sel(pctldev, pgrp->pins[i], pgrp->mux_val);
					...
					unsigned int offset)
{
	...
	ts_tx5336_gpio_func_sel(pctldev, offset, TS_PINMUX_GPIO_MUX_VAL);
				   ...
				   unsigned int pin, unsigned long *config)
{
	...
	case PIN_CONFIG_DRIVE_STRENGTH:
		...
		break;
    case PIN_CONFIG_VOLTAGE:
        mode_offset = SCD_GPIO_MSC_REG;
        reg_offset = 0;
		break;
	default:
		dev_dbg(pctldev->dev,
    ...
    pr_debug("pctrl->reg_base + mode_offset + pad_offset:%p\n", pctrl->reg_base + mode_offset + pad_offset);
				   ...
				   unsigned long *configs, unsigned int num_configs)
{
	...
	for (i = 0; i < num_configs; i++) {
		...
		case PIN_CONFIG_BIAS_PULL_UP:
			mode_offset = SCD_GPIO_PUR_REG;
			/*to config pull up should clear pull down otherwise*/
		...
		case PIN_CONFIG_DRIVE_STRENGTH:
			...
			for(drv_stren_idx=0; drv_stren_idx < DRV_STREN_KINDS; drv_stren_idx++) {
				...
				val = readl(pctrl->reg_base + mode_offset + pad_offset);
                pr_debug("PIN_CONFIG_DRIVE_STRENGTH - read:0x%p val:%d\n", (pctrl->reg_base + mode_offset + pad_offset), val);
				val &= ~(1 << reg_offset);
        ...
        case PIN_CONFIG_VOLTAGE:
            ...
            // pr_debug(" PIN_CONFIG_VOLTAGE set :0x%p  valtage:%s \n", (pctrl->reg_base + SCD_GPIO_MSC_REG + pad_offset), 
            //                                                         "1V8" );
            break;
		default:
		...
		val = readl(pctrl->reg_base + mode_offset + pad_offset);
        pr_debug("reading:0x%p \n val:%d\n", pctrl->reg_base + mode_offset + pad_offset, val);
		val |= 1 << reg_offset;
        pr_debug("writing:0x%p \n val:%d\n", pctrl->reg_base + mode_offset + pad_offset, val);        
		writel(val, pctrl->reg_base + mode_offset + pad_offset);
	}
