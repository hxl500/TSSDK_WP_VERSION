...
#define SRC_IS_INC(ccr) ((ccr) & CC_SRCINC)
#define DST_IS_INC(ccr) ((ccr) & CC_DSTINC)
/*
...
static inline u32 _emit_ADD(unsigned dry_run, u8 buf[],
		enum pl330_dst dst, u32 val)
{
	if (dry_run)
		return SZ_DMAADDH;
	...
	PL330_DBGCMD_DUMP(SZ_DMAMOV, "\tDMAADD %s 0x%x\n",
		dst == SRC ? "SAR" : "DAR", val);

	return SZ_DMAADDH;
}

static inline u32 _emit_ADN(unsigned dry_run, u8 buf[],
		enum pl330_dst dst, u32 val)
{
	if (dry_run)
		return SZ_DMAADDH;
	...
	PL330_DBGCMD_DUMP(SZ_DMAMOV, "\tDMAADN %s 0x%x\n",
		dst == SRC ? "SAR" : "DAR", val);
...
static inline int _ldst_memtomem(unsigned dry_run, u8 buf[],
		const struct _xfer_spec *pxs, int cyc, int elem_cyc)
{
	int off = 0;
	struct pl330_config *pcfg = pxs->desc->rqcfg.pcfg;
	struct pl330_xfer *pxp = &pxs->desc->px;
	u32 ccr = pxs->ccr;
	u32 burst_size = BURST_TO_BYTE(1, ccr);
	u32 ldsts_cnt;
	...
	while (cyc--) {
		...
		if (pxp->seg_num > 0) {
			if (pxp->src_seg_size > burst_size)
				off += _emit_ADD(dry_run, &buf[off], SRC, pxp->src_seg_size - burst_size);
			else if (pxp->src_seg_size < burst_size)
				off += _emit_ADN(dry_run, &buf[off], SRC, burst_size - pxp->src_seg_size);

			if (pxp->dst_seg_size > burst_size)
				off += _emit_ADD(dry_run, &buf[off], DST, pxp->dst_seg_size - burst_size);
			else if (pxp->dst_seg_size < burst_size)
				off += _emit_ADN(dry_run, &buf[off], DST, burst_size - pxp->dst_seg_size);
		}
	}
		...
		const struct _xfer_spec *pxs, int cyc, int elem_cyc)
{
	...
	case DMA_DEV_TO_MEM:
		...
		break;
	case DMA_MEM_TO_MEM:
		off += _ldst_memtomem(dry_run, &buf[off], pxs, cyc, elem_cyc);
		break;
	default:
...
{
	int cyc, elem_cyc, cycmax, szlp, szlpend, szbrst, off;
	int remain, i;
	unsigned lcnt0, lcnt1, ljmp0, ljmp1;
	struct _arg_LPEND lpend;
	struct pl330_xfer *pxp = &pxs->desc->px;
	u32 burst_size = BURST_TO_BYTE(1, pxs->ccr);

	if (*bursts == 1)
		return _bursts(pl330, dry_run, buf, pxs, 1, 1);

	if (pxp->seg_num > 0) {
		if (pxp->seg_num > 256) {
			cyc = (pxp->seg_num + 255) / 256;
			lcnt1 = pxp->seg_num / cyc;
			remain = pxp->seg_num - lcnt1 * cyc;
		} else {
			cyc = 1;
			lcnt1 = pxp->seg_num;
			remain = 0;
		...
		if (pxp->line_num - pxp->line_cnt > 256)
			lcnt0 = 256;
		else
			lcnt0 = pxp->line_num - pxp->line_cnt;

		elem_cyc = pxp->elem_size / burst_size;
		pxp->line_cnt += lcnt0;
	} else {
	...
	pr_debug("lcnt0:%d, lcnt1:%d, cyc:%d, elem_cyc:%d, remain:%d\n", lcnt0, lcnt1, cyc, elem_cyc, remain);

	szlp = _emit_LP(1, buf, 0, 0);
	szbrst = _bursts(pl330, 1, buf, pxs, 1, 1);
	...
	off += _bursts(pl330, dry_run, &buf[off], pxs, cyc, elem_cyc);
	...
	for (i=0; i<remain; i++)
		off += _bursts(pl330, dry_run, &buf[off], pxs, 1, 1);

	if (pxp->seg_num > 0) {
		if (pxp->src_line_size > pxp->src_seg_size*pxp->seg_num)
			off += _emit_ADD(dry_run, &buf[off], SRC, pxp->src_line_size - pxp->src_seg_size*pxp->seg_num);

		if (pxp->dst_line_size > pxp->dst_seg_size*pxp->seg_num)
			off += _emit_ADD(dry_run, &buf[off], DST, pxp->dst_line_size - pxp->dst_seg_size*pxp->seg_num);
	}
						...
						struct of_dma *ofdma)
{
	...
	if (chan_id >= pl330->num_peripherals)
		return dma_get_any_slave_channel(&pl330->ddma);
...
{
	memset(px, 0, sizeof(struct pl330_xfer));
	px->bytes = len;
	px->dst_addr = dst;
	px->src_addr = src;
}
...
static struct dma_async_tx_descriptor *pl330_prep_dma_cyclic(
		struct dma_chan *chan, struct snd_pcm_substream *subtream, size_t len,
...
static struct dma_async_tx_descriptor *
pl330_prep_dma_interval_memcpy(struct dma_chan *chan, struct dma_interval *itrl, unsigned long flags)
{
	struct dma_pl330_desc *desc;
	struct dma_pl330_chan *pch = to_pchan(chan);
	struct pl330_dmac *pl330;
	struct pl330_xfer *pxp;
	int burst;
	size_t len;

	if (unlikely(!pch || !itrl->elem_size))
		return NULL;
	...
	pr_debug("seg_num=%lu, line_num=%lu, elem_size=%lu, src_seg_size=%lu, dst_seg_size=%lu\n",
			itrl->seg_num, itrl->line_num, itrl->elem_size, itrl->src_seg_size, itrl->dst_seg_size);

	pxp = &desc->px;
	pxp->src_seg_size = itrl->src_seg_size;
	pxp->dst_seg_size = itrl->dst_seg_size;
	pxp->src_line_size = itrl->src_line_size;
	pxp->dst_line_size = itrl->dst_line_size;
	pxp->seg_num = itrl->seg_num;
	pxp->line_num = itrl->line_num;
	pxp->src_addr = itrl->src_addr;
	pxp->dst_addr = itrl->dst_addr;
	pxp->elem_size = itrl->elem_size;

	len = itrl->elem_size*itrl->seg_num*itrl->line_num;
	pxp->bytes = len;
	 ...
	 * transfers which straddle an entry in the DMA device's MFIFO.
	 */
	while ((itrl->src_addr | itrl->dst_addr | itrl->elem_size) & (burst - 1))
		burst /= 2;
	...
	desc->rqcfg.brst_len = get_burst_len(desc, itrl->elem_size);
#if 0
	/*
	 * If burst size is smaller than bus width then make sure we only
	 * transfer one at a time to avoid a burst stradling an MFIFO entry.
	...
	if (burst * 8 < pl330->pcfg.data_bus_width)
		desc->rqcfg.brst_len = 1;
#endif
	pr_debug("len:0x%zx, burst size:%u, burst len:%u\n", len ,desc->rqcfg.brst_size, desc->rqcfg.brst_len);
	desc->bytes_requested = len;
...
pl330_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	pd->device_prep_dma_interval_memcpy = pl330_prep_dma_interval_memcpy;
