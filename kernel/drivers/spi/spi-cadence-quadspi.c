...
{
	u32 val = 0, delay_us = 0, max_delay_us = CQSPI_TIMEOUT_MS * 1000;
					...
					const size_t n_tx)
{
	...
	while (remaining > 0) {
		...
		if (write_words) {
			iowrite32_rep(cqspi->ahb_base + cqspi->trig_addr_off, txbuf, write_words);
			txbuf += (write_words * 4);
		}
		if (mod_bytes) {
...
static void cqspi_config_baudrate_div(struct cqspi_st *cqspi)
{
	...
	div = (DIV_ROUND_CLOSEST(ref_clk_hz, cqspi->sclk) / 2) - 1;
	if (IS_ENABLED(CONFIG_MTD_SPI_NAND) && div == 0)
		div = 1;


	printk("ref_clk_hz=%d, cqspi->sclk=%d, div=%d, div_actually=%d, clk_actually=%d\n",
			ref_clk_hz, cqspi->sclk, div, (div + 1) * 2, ref_clk_hz / ((div + 1) * 2));
	reg = readl(reg_base + CQSPI_REG_CONFIG);
...
static int cqspi_mem_process(struct spi_mem *mem, const struct spi_mem_op *op)
{
	...
	pr_debug(" cmd:0x%02x bw 0x%x\n",
		 op->cmd.opcode, op->cmd.buswidth);

	if (op->addr.nbytes > 0) {
		pr_debug(" addr:0x%06llx num 0x%x bw 0x%x\n ",
			 op->addr.val, op->addr.nbytes, op->addr.buswidth);
	}

	if (op->dummy.nbytes > 0) {
		pr_debug(" dummy: num 0x%x bw 0x%x\n",
			  op->dummy.nbytes, op->dummy.buswidth);
	}

	if (op->data.dir == SPI_MEM_DATA_OUT) {
		pr_debug(" data: 0x%08x  num 0x%x bw 0x%x\n",
				    ...
				    struct device_node *np)
{
	if (of_property_read_u32(np, "cdns,read-delay", &f_pdata->read_delay)) {
		dev_warn(&pdev->dev, "couldn't determine read-delay, use default value.\n");
...
{
	const struct cqspi_driver_platdata *ddata;
	struct reset_control *rtsc_ahb, *rtsc_apb, *rtsc_ref;
	struct device *dev = &pdev->dev;
	...
	rtsc_ahb = devm_reset_control_get_optional_exclusive(dev, "ospi_ahb");
	if (IS_ERR(rtsc_ahb)) {
		ret = PTR_ERR(rtsc_ahb);
		dev_err(dev, "Cannot get QSPI AHB reset.\n");
		goto probe_reset_failed;
	}

	rtsc_apb = devm_reset_control_get_optional_exclusive(dev, "ospi_apb");
	if (IS_ERR(rtsc_apb)) {
		ret = PTR_ERR(rtsc_apb);
		dev_err(dev, "Cannot get QSPI APB reset.\n");
		goto probe_reset_failed;
	}

	rtsc_ref = devm_reset_control_get_optional_exclusive(dev, "ospi_ref");
	if (IS_ERR(rtsc_ref)) {
		ret = PTR_ERR(rtsc_ref);
		dev_err(dev, "Cannot get QSPI REF clock reset.\n");
		goto probe_reset_failed;
	}

	reset_control_assert(rtsc_ahb);
	reset_control_assert(rtsc_apb);
	reset_control_assert(rtsc_ref);
	ndelay(100);
	reset_control_deassert(rtsc_ahb);
	reset_control_deassert(rtsc_apb);
	reset_control_deassert(rtsc_ref);
...
static const struct cqspi_driver_platdata tsm_qspi = {
	.quirks = CQSPI_DISABLE_DAC_MODE,
};
