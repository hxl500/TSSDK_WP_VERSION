...
#define RX_BUSY		0
#define RX_BURST_LEVEL	0
#define TX_BUSY		1
#define TX_BURST_LEVEL	0

#define DMA_MEM_BURST_LEN	4	/* 4 data item for DDR meomry */

static int dma_enable = 1;
module_param(dma_enable, int, 0644);
MODULE_PARM_DESC(dma_enable, "dw ssi dma xfer enable");
...
static void dw_spi_dma_sg_burst_init(struct dw_spi *dws)
{
	...
	if (tx.max_burst > 0 && rx.max_burst > 0)
		dws->dma_sg_burst = min(tx.max_burst, rx.max_burst);
	else if (tx.max_burst > 0)
		dws->dma_sg_burst = tx.max_burst;
	else if (rx.max_burst > 0)
		dws->dma_sg_burst = rx.max_burst;
	else
		dws->dma_sg_burst = 0;
}
...
static int dw_spi_dma_wait(struct dw_spi *dws, unsigned int len, u32 speed)
{
	...
	ms *= 10; /* multiply 10 for time sequence */
...
static int dw_spi_dma_config_tx(struct dw_spi *dws)
{
	...
	/* dst_maxburst & src_maxburst may not be used by DAMC */
	txconf.dst_maxburst = dws->txburst;
	txconf.src_maxburst = DMA_MEM_BURST_LEN;
...
static int dw_spi_dma_config_rx(struct dw_spi *dws)
{
	...
	rxconf.dst_maxburst = DMA_MEM_BURST_LEN;
