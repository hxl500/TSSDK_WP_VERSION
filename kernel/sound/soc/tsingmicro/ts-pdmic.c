/* Tsingmicro PDMIC driver
 *
 * Copyright (C) 2015 Atmel
 *
 * Author: Sheng Yang <yangsheng@tsingmicro.com>
 *
...
static int ts_pdmic_asoc_card_init_times = 0;

int ts_pdmic_card_number[4] = {
	TS_CODEC_PDMIC_CARD_IDX,
	TS_CODEC_PDMIC_CARD_IDX7,
	TS_CODEC_PDMIC_CARD_IDX8,
	TS_CODEC_PDMIC_CARD_IDX9
};
...
static struct ts_pdmic_pdata *ts_pdmic_dt_init(struct device *dev)
{
	...
	if (of_property_read_u32(np, "ts,mic-min-freq",
				 &pdata->mic_min_freq)) {
	...
	if (of_property_read_s32(np, "ts,mic-offset", &pdata->mic_offset))
		pdata->mic_offset = 0;
...
/* cpu dai component */
static int ts_pdmic_cpu_dai_startup(struct snd_pcm_substream *substream,
					struct snd_soc_dai *cpu_dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct ts_pdmic *dd = snd_soc_card_get_drvdata(rtd->card);
	int ret;

#if 1
	ret = clk_prepare_enable(dd->mclk);
	if (ret)
		return ret;
	...
	regmap_update_bits(dd->regmap, PDMIC_FIFO_INTEN, PDMIC_FIFO_INTEN_OVERFLOW_MASK,
		PDMIC_FIFO_INTEN_EN << PDMIC_FIFO_INTEN_OVERFLOW_SHIFT);

	return 0;
}

static void ts_pdmic_cpu_dai_shutdown(struct snd_pcm_substream *substream,
					struct snd_soc_dai *cpu_dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct ts_pdmic *dd = snd_soc_card_get_drvdata(rtd->card);
	...
	regmap_update_bits(dd->regmap, PDMIC_FIFO_INTEN, PDMIC_FIFO_INTEN_OVERFLOW_MASK,
			PDMIC_FIFO_INTEN_DIS << PDMIC_FIFO_INTEN_OVERFLOW_SHIFT);

	clk_disable_unprepare(dd->mclk);
}

static int ts_pdmic_cpu_dai_prepare(struct snd_pcm_substream *substream,
					struct snd_soc_dai *cpu_dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct ts_pdmic *dd = snd_soc_card_get_drvdata(rtd->card);
	...
	return regmap_read(dd->regmap, PDMIC_FIFO_RD, &val);
}

static const struct snd_soc_dai_ops ts_pdmic_cpu_dai_ops = {
	.startup	= ts_pdmic_cpu_dai_startup,
	.shutdown	= ts_pdmic_cpu_dai_shutdown,
	.prepare	= ts_pdmic_cpu_dai_prepare,
};

//#define TS_PDMIC_FORMATS (SNDRV_PCM_FMTBIT_U16_LE | SNDRV_PCM_FMTBIT_S16_LE
//| SNDRV_PCM_FMTBIT_S32_LE | SNDRV_PCM_FMTBIT_U32_LE)
#define TS_PDMIC_FORMATS (SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_U16_LE)
static struct snd_soc_dai_driver ts_pdmic_cpu_dai = {
	.capture = {
		.channels_min	= 1,
		.channels_max	= 4,
		.rates		= SNDRV_PCM_RATE_KNOT,
		.formats	= TS_PDMIC_FORMATS,},
	.ops = &ts_pdmic_cpu_dai_ops,
};

static const struct snd_soc_component_driver ts_pdmic_cpu_dai_component = {
	.name = "tsingmicro-pdmic",
};

/* platform */
#define TS_PDMIC_MAX_BUF_SIZE  (64 * 1024)
#define TS_PDMIC_PREALLOC_BUF_SIZE  TS_PDMIC_MAX_BUF_SIZE

static const struct snd_pcm_hardware ts_pdmic_hw = {
	...
	.formats		= TS_PDMIC_FORMATS,
	.buffer_bytes_max	= TS_PDMIC_MAX_BUF_SIZE,
	.period_bytes_min	= 256,
	.period_bytes_max	= 4 * 1024,	//32 * 1024
	.periods_min		= 2,
	.periods_max		= 256,
};

static int
ts_pdmic_platform_configure_dma(struct snd_pcm_substream *substream,
				...
				struct dma_slave_config *slave_config)
{
	...
	slave_config->src_addr		= dd->phy_base + PDMIC_FIFO_RD;
	slave_config->src_maxburst	= 4;//size=4 -> reg velue=1;
	slave_config->dst_maxburst	= 4;//size=4 -> reg velue=1;

	return 0;
}

static const struct snd_dmaengine_pcm_config
ts_pdmic_dmaengine_pcm_config = {
	.prepare_slave_config	= ts_pdmic_platform_configure_dma,
	.pcm_hardware		= &ts_pdmic_hw,
	.prealloc_buffer_size	= TS_PDMIC_PREALLOC_BUF_SIZE,
};
...
static const struct snd_kcontrol_new ts_pdmic_snd_controls[] = {
...
SOC_SINGLE("High Pass Filter Switch", PDMIC_CFG,
	   PDMIC_CFG_HPF_SHIFT, 1, 1),

// SOC_SINGLE("SINCC Filter Switch", PDMIC_DSPR0, PDMIC_DSPR0_SINBYP_SHIFT, 1, 1),
};

static int ts_pdmic_codec_probe(struct snd_soc_codec *codec)
{
	// struct snd_soc_card *card = snd_soc_codec_get_drvdata(codec);
	// struct ts_pdmic *dd = snd_soc_card_get_drvdata(card);
...
static struct snd_soc_codec_driver soc_codec_dev_pdmic = {
	.probe		= ts_pdmic_codec_probe,
	.component_driver = {
		.controls		= ts_pdmic_snd_controls,
		.num_controls		= ARRAY_SIZE(ts_pdmic_snd_controls),
	},
};
...
static int
ts_pdmic_codec_dai_hw_params(struct snd_pcm_substream *substream,
			    struct snd_pcm_hw_params *params,
			    struct snd_soc_dai *codec_dai)
{
	...
	//pr_err("%s %d,rate_min = %d\n", __func__, __LINE__,rate_min);
	//pr_err("%s %d,rate_max = %d\n", __func__, __LINE__,rate_max);
	//pr_err("%s %d,fs = %d\n", __func__, __LINE__,fs);
	//pr_err("%s %d,bits = %d\n", __func__, __LINE__,bits);
	//pr_err("%s %d,channels = %d\n", __func__, __LINE__,channels);
	...
	/*config clk*/
	// mclk_rate = clk_get_rate(dd->mclk);

	/*div to get sample rate*/
	clk_set_rate(dd->mclk, fs*64*4); //set pdm_mclk

	snd_soc_update_bits(codec, PDMIC_CFG,
		PDMIC_CFG_STOREMODE_MASK | PDMIC_CFG_PCMWD_MASK |
		PDMIC_CFG_SMPLEDGE_MASK | PDMIC_CFG_STEREO_MASK, cfg_val);

	return 0;
}

static int ts_pdmic_codec_dai_prepare(struct snd_pcm_substream *substream,
					struct snd_soc_dai *codec_dai)
{
	struct snd_soc_codec *codec = codec_dai->codec;

	/*enable dma*/
	snd_soc_update_bits(codec, PDMIC_FIFO_CFG, PDMIC_FIFO_CFG_DMAEN_MASK,
			PDMIC_FIFO_CFG_EN << PDMIC_FIFO_CFG_DMAEN_SHIFT);

	/*config dma trig thredhold*/
	snd_soc_update_bits(codec, PDMIC_FIFO_CFG, PDMIC_FIFO_CFG_TRIGLVL_MASK,
			EIGHT << PDMIC_FIFO_CFG_TRIGLVL_SHIFT);

	/*enable HPF to remove dc */
	snd_soc_update_bits(codec, PDMIC_CFG, PDMIC_CFG_HPF_MASK,
			PDMIC_CFG_HPF_EN << PDMIC_CFG_HPF_SHIFT);

	snd_soc_update_bits(codec, PDMIC_ENABLE, PDMIC_ENABLE_PDMEN_MASK,
			PDMIC_ENABLE_PDMEN_DIS << PDMIC_ENABLE_PDMEN_SHIFT);

	return 0;
}

static int ts_pdmic_codec_dai_trigger(struct snd_pcm_substream *substream,
					int cmd, struct snd_soc_dai *codec_dai)
{
	...
	snd_soc_update_bits(codec, PDMIC_ENABLE, PDMIC_ENABLE_PDMEN_MASK, val);

	return 0;
}

static const struct snd_soc_dai_ops ts_pdmic_codec_dai_ops = {
	.hw_params	= ts_pdmic_codec_dai_hw_params,
	.prepare	= ts_pdmic_codec_dai_prepare,
	.trigger	= ts_pdmic_codec_dai_trigger,
};

#define TS_PDMIC_CODEC_DAI_NAME  "ts-pdmic-hifi"

static struct snd_soc_dai_driver ts_pdmic_codec_dai = {
	.name = TS_PDMIC_CODEC_DAI_NAME,
	.capture = {
		.stream_name	= "Capture",
		.channels_min	= 1,
		.channels_max	= 4,
	...
	.ops = &ts_pdmic_codec_dai_ops,
};

/* ASoC sound card */
static int ts_pdmic_asoc_card_init(struct device *dev,
				struct snd_soc_card *card)
{
	...
	dai_link->codec_dai_name	= TS_PDMIC_CODEC_DAI_NAME;
	...
	card->dev	= dev;
	// card->card_number	= TS_CODEC_PDMIC_CARD_IDX;
	card->card_number	= ts_pdmic_card_number[ts_pdmic_asoc_card_init_times++];//TS_CODEC_PDMIC_CARD_IDX;

	return 0;
}

static void ts_pdmic_get_sample_rate(struct ts_pdmic *dd,
	unsigned int *rate_min, unsigned int *rate_max)
{
	...
	/*depend on pdm_mclk*/
	*rate_min = mic_min_freq >> 6; // 1.024M/64=16k
	*rate_max = mic_max_freq >> 6; // 3.072M/64=48k
}

/* PDMIC interrupt handler */
static irqreturn_t ts_pdmic_interrupt(int irq, void *dev_id)
{
	struct ts_pdmic *dd = (struct ts_pdmic *)dev_id;
	u32 pdmic_isr;
	irqreturn_t ret = IRQ_NONE;

	regmap_read(dd->regmap, PDMIC_FIFO_STAT, &pdmic_isr);

	if (pdmic_isr & (PDMIC_FIFO_STAT_OVERFLOW | PDMIC_FIFO_STAT_UNDERFLOW)) {
		regmap_update_bits(dd->regmap, PDMIC_ENABLE, PDMIC_ENABLE_PDMEN_MASK,
				   PDMIC_ENABLE_PDMEN_DIS << PDMIC_ENABLE_PDMEN_SHIFT);
...
static int ts_pdmic_probe(struct platform_device *pdev)
{
...
#if 1
	dd->pclk = devm_clk_get(dev, "pdm_pclk");
	if (IS_ERR(dd->pclk)) {
		ret = PTR_ERR(dd->pclk);
		dev_err(dev, "failed to get pdm_pclk: %d\n", ret);
		return ret;
	}
	...
	dd->mclk = devm_clk_get(dev, "pdm_mclk");
	if (IS_ERR(dd->mclk)) {
		ret = PTR_ERR(dd->mclk);
		dev_err(dev, "failed to get pdm_mclk: %d\n", ret);
		return ret;
	}
#endif
	...
	ret =  devm_request_irq(dev, dd->irq, ts_pdmic_interrupt, 0,
				"PDMIC", (void *)dd);
	if (ret < 0) {
	...
	ts_pdmic_get_sample_rate(dd, &rate_min, &rate_max);

	/* register cpu dai */
	ts_pdmic_cpu_dai.capture.rate_min = rate_min;
	ts_pdmic_cpu_dai.capture.rate_max = rate_max;
	ret = devm_snd_soc_register_component(dev,
					      &ts_pdmic_cpu_dai_component,
					      &ts_pdmic_cpu_dai, 1);
	if (ret) {
	...
	ts_pdmic_codec_dai.capture.rate_min = rate_min;
	ts_pdmic_codec_dai.capture.rate_max = rate_max;
	ret = snd_soc_register_codec(dev, &soc_codec_dev_pdmic,
				     &ts_pdmic_codec_dai, 1);
	if (ret) {
	...
	ret = ts_pdmic_asoc_card_init(dev, card);
	if (ret) {
