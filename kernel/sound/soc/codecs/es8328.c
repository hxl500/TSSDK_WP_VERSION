...
static int es8328_parse_dt(struct device *dev,
				struct es8328_priv *es8328)
{
	es8328->ce_gpio = devm_gpiod_get(dev, "ce", GPIOD_OUT_HIGH);
	if (!es8328->ce_gpio) {
		dev_err(dev, "devm_gpiod_get failed.\n");
		return -1;
	}

	gpiod_set_value(es8328->ce_gpio, 1);
	pr_err("gpiod_set_value 1\n");
	return 0;
}
